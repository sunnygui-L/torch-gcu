...
std::vector<std::pair<Backend, ScalarType>> all_declared_types_gcu() {
  ...
  std::vector<ScalarType> scalar_types = {
      ScalarType::Byte,    ScalarType::Char, ScalarType::Double,
...
struct PyTensorType {
  ...
  Backend get_backend() const { return static_cast<Backend>(backend); }
...
static torch::TypeError unavailable_type(const PyTensorType& type) {
  return torch::TypeError(
      ...
      " not available. Torch not compiled with gcu enabled.");
}
...
static PyObject* Tensor_new(PyTypeObject* type, PyObject* args,
  ...
  if (tensor_type.is_gcu) {
    TORCH_WARN_ONCE(
        "Warning: The torch.gcu.*DtypeTensor constructors are no longer "
        "recommended. It's best to use methods such as torch.tensor(data, "
        "dtype=*, device='gcu') to create tensors.");
  }
  if (tensor_type.is_gcu && torch_gcu::device_count() == 0) {
...
PyObject* Tensor_is_gcu(PyTensorType* self, void* /*unused*/) {
  if (self->is_gcu) {
    Py_RETURN_TRUE;
  } else {
    Py_RETURN_FALSE;
...
static struct PyMethodDef metaclass_methods[] = {
    {"__instancecheck__", Tensor_instancecheck, METH_O, nullptr}, {nullptr}};
...
static struct PyGetSetDef metaclass_properties[] = {
    ...
    {"is_gcu", (getter)Tensor_is_gcu, nullptr, nullptr, nullptr},
...
static void py_initialize_tensor_type(PyTypeObject& type, const char* name,
                                      PyObject* tp_dict) {
...
static void set_type(PyTensorType& type_obj, Backend backend,
  ...
  type_obj.is_gcu = (backend == c10::Backend::PrivateUse1);
}
...
static void initialize_gcu_aten_types(std::vector<PyTensorType>& tensor_types) {
  // only initialize gcu types
  auto declared_types = all_declared_types_gcu();
...
void initialize_python_bindings() {
  ...
  initialize_gcu_aten_types(tensor_types);
  ...
  // `torch.gcu.FloatTensor.add`.
  auto tensor_dict = get_tensor_dict();

  // Initialize each Python type object torch.gcu.FloatTensor,
  // torch.gcu.DoubleTensor, etc.
  for (auto& tensor_type : tensor_types) {
    py_initialize_tensor_type(tensor_type.py_type, tensor_type.name,
                              tensor_dict.get());
  }

  // Add the type objects to their corresponding modules. e.g.
  // torch.gcu.FloatTensor is added to the `torch_gcu` module as `FloatTensor`.
  // Also add all the type objects to the set torch_gcu._tensor_classes.
...
// classes
static PyObject* THPModule_initGcuTensorType(PyObject* /*_unused*/,
                                             PyObject* /*noargs*/) {
  HANDLE_TH_ERRORS
...
static PyMethodDef TorchGcuTensorTypeMethods[] = {
    {"_initGcuTensorType", (PyCFunction)THPModule_initGcuTensorType,
     METH_NOARGS, nullptr},
    {nullptr, nullptr, 0, nullptr}};

PyMethodDef* gcu_tensor_type_functions() { return TorchGcuTensorTypeMethods; }
