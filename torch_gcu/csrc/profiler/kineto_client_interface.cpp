...
class LibKinetoClient : public libkineto_gcu::ClientInterface {
 public:
  void init() override {}

  void prepare(bool report_input_shapes = false, bool profile_memory = false,
               bool with_stack = false, bool with_flops = false,
  ...
  void start() override {
    ProfilerConfig cfg{ProfilerState::KINETO_ONDEMAND,
...
struct RegisterLibKinetoClient {
  RegisterLibKinetoClient() {
    ...
    libkineto_gcu::api().registerClient(&client);
    ...
    if (std::getenv("KINETO_USE_DAEMON") != nullptr) {
      ...
      if (init_delay_s > 0) {
        std::thread t([init_delay_s, kineto_init]() {
          torch_gcu::util::setThreadName("TorchKinetoInit");
