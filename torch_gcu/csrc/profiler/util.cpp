...
bool softAssertRaises() { return soft_assert_raises_.value_or(false); }

void logSoftAssert(const char* func, const char* file, uint32_t line,
                   const char* cond, const char* args) {
  std::string error;
  error = fmt::format("{} SOFT ASSERT FAILED at {}:{}, func: {}, args: {}",
                      cond, file, line, func, args);
  // TODO: Implement profile_id and group_profile_id as 3rd/4th arguments.
  kineto::logInvariantViolation(cond, error, "", "");
}

void logSoftAssert(const char* func, const char* file, uint32_t line,
                   const char* cond, const std::string& args) {
  std::string error;
  error = fmt::format("{} SOFT ASSERT FAILED at {}:{}, func: {}, args: {}",
                      cond, file, line, func, args);
  // TODO: Implement profile_id and group_profile_id as 3rd/4th arguments.
...
std::string getNvtxStr(
  ...
  if (sequence_nr >= -1 || !shapes.empty()) {
    ...
    if (!input_op_ids.empty()) {
      str = fmt::format("{}, input_op_ids = {}", str,
                        inputOpIdsToStr(input_op_ids));
    }
    return str;
  } else {
    return name;
...
std::vector<FileLineFunc> prepareCallstack(
    const std::vector<torch::jit::StackEntry>& cs) {
...
std::string inputOpIdsToStr(
  ...
  for (const auto& op_id_info_pair : input_op_ids) {
    ...
    // (OpId,OutputNr)
    str = fmt::format("{}({},{})", str, op_id_info_pair.first,
                      op_id_info_pair.second);
  }
  str = fmt::format("{}]", str);
  return str;
}
...
std::unordered_map<std::string, std::string> saveNcclMeta(
    const at::RecordFunction& fn, bool truncate) {
  ...
  map.emplace(kDtype,
              fmt::format("\"{}\"", c10::toString(debugInfo->getDType())));
...
static std::vector<c10::IntArrayRef> getInputSizes(
    const std::string& op_name, size_t min_size,
...
std::unordered_map<std::string, c10::IValue> saveExtraArgs(
  ...
  if (fname == kConv2dOp) {
    ...
    if (inputSizes[1].size() != 4) {
      TORCH_WARN(
          "Failed to compute flops for op aten::conv2d because it requires a "
          "4D kernel tensor.");
      return map;
    }
...
uint64_t computeFlops(
  ...
  if (op_name == kConv2dOp) {
    if (extra_args.find(kInputSize) == extra_args.end() ||
      ...
      TORCH_WARN(
          "Calculating flops for aten::conv2d requires groups, padding, "
          "stride, dilation, input_size, and weight_size in saved arguments.");
      return 0;
    }
    ...
    if (!input_sizes_ref.isIntList() || !kernel_sizes_ref.isIntList()) {
      TORCH_WARN(
          "Failed to compute flops for op aten::conv2d because it requires "
          "input and weight tensor sizes.");
      return 0;
    }
    if (!padding_ref.isIntList() || !stride_ref.isIntList() ||
      ...
      TORCH_WARN(
          "Failed to compute flops for op aten::conv2d because it requires "
          "padding, stride, and dilation values.");
      return 0;
    }
    ...
    if (input_sizes.size() != 4 || kernel_sizes.size() != 4) {
      TORCH_WARN(
          "Failed to compute flops for op aten::conv2d because both input and "
          "weight must be size 4.");
      return 0;
    }
    if (!groups) {
      TORCH_WARN(
          "Failed to compute flops for op aten::conv2d because group size must "
          "not be 0.");
      return 0;
    }
    if (padding.size() != 2 || dilation.size() != 2) {
      TORCH_WARN(
          "Failed to compute flops for op aten::conv2d because both padding "
          "and dilation must be size 2.");
      return 0;
    }
    if (stride.size() != 2 || (stride[0] * stride[1] == 0)) {
      TORCH_WARN(
          "Failed to compute flops for op aten::conv2d because stride must be "
          "size 2 and cannot be 0.");
      return 0;
    }
    ...
    if (extra_args.find(kMat1Size) == extra_args.end() ||
      ...
      TORCH_WARN("Calculating flops for ", op_name,
    ...
    if (!mat1_sizes_ref.isIntList() || !mat2_sizes_ref.isIntList()) {
      TORCH_WARN("Failed to compute flops for op ", op_name,
    ...
    if (extra_args.find(kMatSize) == extra_args.end()) {
      TORCH_WARN(
          "Calculating flops for aten::mul.Tensor requires mat_size in saved "
          "arguments.");
      return 0;
    }
    ...
    if (!mat_sizes.isIntList()) {
      TORCH_WARN(
          "Failed to compute flops for op aten::mul because it requires "
          "mat_size to be IntList.");
      return 0;
    }
    ...
    if (extra_args.find(kMatSize) == extra_args.end()) {
      TORCH_WARN(
          "Calculating flops for aten::add.Tensor requires mat_size in saved "
          "arguments.");
      return 0;
    }
    auto mat_sizes = extra_args.at(kMatSize);
    if (!mat_sizes.isIntList()) {
      TORCH_WARN(
          "Failed to compute flops for op aten::add because it requires "
          "mat_size to be IntList.");
      return 0;
    }
