...
struct TORCH_API ExecutionTraceObserver {  // NOLINT
  ...
  ID getNewID() { return id_++; }

  RunState getState() const { return state_; }
...
static inline std::string getAttrJson(const std::string& name,
...
static void writeJsonNode(
    std::ofstream& out, const std::string& name, const uint64_t id,
    const uint64_t rf_id, const uint64_t parent, const uint64_t fw_parent,
    const int64_t seq_id, const uint64_t scope, const uint64_t tid,
    const uint64_t fw_tid, const std::string& inputs = "[]",
    const std::string& inputShapes = "[]",
    const std::string& inputStrides = "[]",
    const std::string& inputTypes = "[]", const std::string& outputs = "[]",
    ...
    const std::string& kernelBackend = "", const std::string& kernelFile = "",
  ...
  out << fmt::format(
    ...
    {{
      ...
      id, name, parent, inputs, inputShapes, inputTypes, inputStrides, outputs,
      output_shapes, output_types, output_strides, rf_id, fw_parent, seq_id,
      scope, tid, fw_tid, operator_schema, kernelBackend, kernelFile,
      additional_attrs);
}
...
static bool initExecutionTraceStart(ExecutionTraceObserver& ob) {
  ...
  "nodes": [)JSON",
      ob.pid, ob.recordTime, timestamp);
  return true;
}
...
static void finalizeExecutionTraceOutput(ExecutionTraceObserver& ob) {
  writeJsonNode(
      ob.out, "[pytorch|profiler|execution_trace|process]", kRootId,
      0,        // rf_id
...
inline ExecutionTraceObserver::ID getObjectID(ExecutionTraceObserver& ob,
                                              const void* t) {
...
convertIValue(ExecutionTraceObserver& ob, const c10::IValue& val,
  ...
  if (val.isTensor()) {
    ...
    tensor_value = fmt::format("[{},{},{},{},{},\"{}\"]", tensor_id, storage_id,
                               offset, numel, itemsize, device_str);
    return std::make_tuple(tensor_shape, tensor_stride, tensor_type,
                           tensor_value);
  } else if (val.isTuple()) {
    ...
    return std::make_tuple(vectorToString(shape_array),
                           vectorToString(stride_array), tensor_type,
...
inline void appendValueInfo(ExecutionTraceObserver& ob, const c10::IValue& val,
...
inline void handleKernelBackendInfo(FunctionCallContext& fc,
  ...
  if (kwinputs.find("kernel_backend") != kwinputs.end()) {
    ...
    if (fc.kernelBackend == "triton") {
      ...
      TORCH_INTERNAL_ASSERT(kwinputs.find("kernel_file") != kwinputs.end(),
      ...
      TORCH_INTERNAL_ASSERT(kwinputs.find("grid") != kwinputs.end(),
                            "grid is missing in triton kernel");
      fc.inputValues.emplace_back("\"" + kwinputs.at("grid").toStringRef() +
                                  "\"");
      fc.inputTypes.emplace_back("\"String\"");
      fc.inputShapes.emplace_back("[]");

      // get stream information
      TORCH_INTERNAL_ASSERT(kwinputs.find("stream") != kwinputs.end(),
...
inline std::string getCommsNodeAttrs(const RecordFunction& fn) {  // NOLINT
  ...
  auto addAttr = [&](const char* commsMetaName, const char* etMetaName,
                     const char* type) {
...
static void recordOperatorStart(ExecutionTraceObserver& ob,
  ...
  try {
    ...
    if (ob.opStack[tid].empty()) {
      ...
      writeJsonNode(ob.out, "[pytorch|profiler|execution_trace|thread]",
                    thread_node_id,
                    0,  // rf_id
                    kRootId,
                    0,   // fw_parent
    ...
    for (const auto i : c10::irange(input_start, inputs.size())) {
      appendValueInfo(ob, inputs[i], fc.inputShapes, fc.inputStrides,
                      fc.inputTypes, fc.inputValues);
    }
  ...
  if (ob->getState() == RunState::enabled) {
    ...
    try {
      ...
      for (const auto i : c10::irange(output_start, outputs.size())) {
        appendValueInfo(*ob, outputs[i], output_shapes, output_strides,
                        output_types, output_values);
      }
      ...
      writeJsonNode(
          ob->out, fc.name, fc.opId, fn.handle(), fc.parentId, fc.fwParentId,
          fn.seqNr(),
          static_cast<std::underlying_type_t<RecordScope>>(fn.scope()),
          fn.threadId(), fn.forwardThreadId(), vectorToString(fc.inputValues),
          vectorToString(fc.inputShapes), vectorToString(fc.inputStrides),
          vectorToString(fc.inputTypes), vectorToString(output_values),
          vectorToString(output_shapes), vectorToString(output_strides),
          vectorToString(output_types), op_schema_str, fc.kernelBackend,
          fc.kernelFile, additional_attrs);
      ob->out << ",";
    } catch (const std::exception& e) {
...
void removeExecutionTraceObserver() {
  ...
  if (ob != nullptr) {
    ...
    if (ob->cbHandle != INVALID_CALLBACK_HANDLE) {
      ...
      TORCH_INTERNAL_ASSERT(ObserverManager::pop() != nullptr,
...
void enableExecutionTraceObserver() {
  ...
  if (ob.getState() == ExecutionTraceObserver::RunState::enabled) {
    LOG(WARNING) << "Trying to enable Execution Trace Observer when it's "
                    "already enabled.";
  } else {
...
void disableExecutionTraceObserver() {
  ...
  if (ob.getState() != ExecutionTraceObserver::RunState::disabled) {
    ...
    LOG(WARNING) << "Trying to disable Execution Trace Observer when it's "
                    "already disabled.";
  }
}
}  // namespace torch_gcu::profiler::impl
