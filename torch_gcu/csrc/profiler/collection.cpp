...
#include <torch_gcu/csrc/profiler/data_flow.h>
...
namespace torch_gcu::profiler::impl {
...
RawTensorMetadataBase::RawTensorMetadataBase(const at::Tensor& t)
  ...
  TORCH_INTERNAL_ASSERT_DEBUG_ONLY(
      ...
      "Tensor has mismatching sizes and strides. Sizes: ", t.sizes().size(),
      " Strides: ", t.strides().size());
}
...
TensorMetadata::TensorMetadata(const RawTensorMetadata& r,
  ...
  return [this, tag_it = tags_.begin(),
    ...
    auto decode_tensor = [&]() -> TensorMetadata {
      ...
      if (tensor_metadata_it.exhausted()) {
        LOG(WARNING) << "Tensor metadata exhausted prematurely. Reported "
                        "shapes may be inaccurate!";
      ...
      for (C10_UNUSED const auto _ : c10::irange(raw_metadata.size_dim_)) {
        if (tensor_size_strides_it.exhausted()) {
          LOG(WARNING) << "Expected Tensor Size mismatch with raw Tensor "
                          "metadata. Reported shapes may be inaccurate!";
      ...
      if (raw_metadata.layout_ == at::kStrided) {
        for (C10_UNUSED const auto _ : c10::irange(raw_metadata.size_dim_)) {
          if (tensor_size_strides_it.exhausted()) {
            LOG(WARNING) << "Expected Tensor Strides mismatch with raw Tensor "
                            "metadata. Reported shapes may be inaccurate!";
            return {raw_metadata, sizes, strides};
          }
...
template <typename T, size_t ChunkSize>
uint64_t
ThreadLocalSubqueue::TorchOpStorage::EventBlock<T, ChunkSize>::correlation_id(
    const T* ptr) const {
...
std::unique_ptr<KinetoObserverContext> ThreadLocalSubqueue::begin_op(
  ...
  auto [event, corr_id] = torch_ops_.op_events_.emplace_back(
      torch_gcu::profiler::impl::TorchOpBasicFields{
          fn.seqNr(), fn.forwardThreadId(), fn.scope(), fn.isAsync(),
          fn.handle(), fn.debugHandle(), fn.name()});
  ...
  if (fn.scope() == at::RecordScope::USER_SCOPE) {
    torch_gcu::profiler::impl::kineto::pushUserCorrelationId(corr_id);
  } else {
    torch_gcu::profiler::impl::kineto::pushCorrelationId(corr_id);
  }
  ...
  if (config_.with_stack && fn.scope() != at::RecordScope::BACKWARD_FUNCTION) {
    auto cs = torch_gcu::profiler::impl::prepareCallstack(
        torch::jit::currentCallstack());
...
void ThreadLocalSubqueue::TorchOpStorage::materialize(
    ...
    const uint64_t tid, const kineto::DeviceAndResource& kineto_info) {
  ...
  const std::string accumulate_grad = "torch_gcu::autograd::AccumulateGrad";
  ...
  for (auto event = op_events_.begin(); event != op_events_.end(); ++event) {
    ...
    if (e.name_.find(profilerStepString) != std::string::npos) {
      step_info.emplace_back(time_converter(event->start_time_),
                             time_converter(event->end_time_), out.size());
    }
...
int64_t torchOpEndNS(const ExtraFields<EventType::TorchOp>& e,
...
auto kinetoEventCorrelationID(const ExtraFields<EventType::Kineto>& e,
...
std::string Result::name() const {
  return visit(
      c10::overloaded(ATTRIBUTE(Vulkan, std::string(e.name_)),
...
libkineto_gcu::ActivityType Result::kinetoType() const {
  return visit(c10::overloaded(
      ATTRIBUTE(TorchOp, scopeToType(e.scope_)),
      ATTRIBUTE(Backend, scopeToType(e.scope_)),
      ATTRIBUTE(Vulkan, libkineto_gcu::ActivityType::CPU_OP),
      ATTRIBUTE(Allocation, libkineto_gcu::ActivityType::CPU_INSTANT_EVENT),
      ATTRIBUTE(OutOfMemory, libkineto_gcu::ActivityType::CPU_INSTANT_EVENT),
      ATTRIBUTE(PyCall, libkineto_gcu::ActivityType::PYTHON_FUNCTION),
      ATTRIBUTE(PyCCall, libkineto_gcu::ActivityType::PYTHON_FUNCTION),
      ATTRIBUTE(Kineto, e.activity_type_)));
}

uint64_t Result::correlationID() const {
  return visit(
      c10::overloaded(ATTRIBUTE(TorchOp, e.correlation_id_),
...
int64_t Result::endTimeNS() const {
  auto end_time_ns = visit(c10::overloaded(
      ...
      ATTRIBUTE(Vulkan,
                start_time_ns_ + (e.in_tree_building_ ? 0 : e.duration_ns_)),
...
uint64_t Result::endTID() const {
  return visit(
      c10::overloaded(ATTRIBUTE(TorchOp, e.end_tid_),
                      [&](const auto&) -> uint64_t { return start_tid_; }));
}

c10::DeviceType Result::deviceType() const {
  using torch_gcu::autograd::profiler::deviceTypeFromActivity;
  return visit(c10::overloaded(
...
ThreadLocalSubqueue::ThreadLocalSubqueue(const uint64_t tid,
                                         ProfilerConfig config)
    : tid_{tid},
      config_{std::move(config)},
      kineto_info_{kineto::kineto_ids()} {
  torch_gcu::profiler::impl::kineto::recordThreadInfo();
  if (!config_.experimental_config.performance_events.empty()) {
    perf_profiler_ =
        std::make_unique<torch_gcu::profiler::impl::linux_perf::PerfProfiler>();
...
void generateForwardBackwardLink(
    const Result& profiler_result, uint64_t& fwd_bwd_link_id,
    libkineto_gcu::GenericTraceActivity& activity,
    std::unordered_map<uint64_t, libkineto_gcu::GenericTraceActivity*>&
        tidSeq2activity) {
  ...
  if (extra_fields.forward_tid_ > 0) {
    // act is backward op.
    uint64_t key = getForwardThreadKey(extra_fields.forward_tid_,
                                       extra_fields.sequence_number_);
    auto iter = tidSeq2activity.find(key);
    if (iter != tidSeq2activity.end()) {
      libkineto_gcu::GenericTraceActivity* fwd = iter->second;
      fwd->flow.start = true;
      activity.flow.id = fwd->flow.id = fwd_bwd_link_id;
      activity.flow.type = fwd->flow.type = libkineto_gcu::kLinkFwdBwd;
      ++fwd_bwd_link_id;
    ...
    // act is forward op.
    uint64_t key = getForwardThreadKey(profiler_result.start_tid_,
                                       extra_fields.sequence_number_);
...
void generateForwardBackwardLinks(
    std::unique_ptr<torch_gcu::profiler::impl::kineto::trace_t>& cpu_trace,
  ...
  using result_activity_t =
      std::pair<Result*, libkineto_gcu::GenericTraceActivity*>;
  ...
  std::sort(torch_events.begin(), torch_events.end(),
            [](const result_activity_t& left, const result_activity_t& right) {
              auto left_end_time = std::get<ExtraFields<EventType::TorchOp>>(
                                       left.first->extra_fields_)
                                       .end_time_ns_;
  ...
  for (auto& [profiler_result, activity] : torch_events) {
    generateForwardBackwardLink(*profiler_result, fwd_bwd_link_id, *activity,
                                tidSeq2activity);
  }
}
...
void passEventsToKineto(const std::vector<std::shared_ptr<Result>>& results,
  ...
  using namespace torch_gcu::profiler::impl::kineto;
  TraceWrapper cpu_trace(static_cast<int64_t>(start_time_ns),
                         "PyTorch Profiler");
...
//   `topsLaunchKernel`   (GCU runtime event, collected by Kineto)
//   `at::vectorized_...` (GCU kernel, collected by Kineto)
// If we only relied on correlation IDs we would set both Kineto events as
// children of the `at::add`, rather than the correct
//   `at::add -> topsLaunchKernel -> at::vectorized_...`
//
// Kineto surfaces this information through a second concept called a "flow".
// In this example, the `topsLaunchKernel` event is the start of a flow and the
// GCU kernel has the same flow id but is not a start event. Thus, when merging
...
class TransferEvents {
  using itrace_t = libkineto_gcu::ITraceActivity;
  using activity_t = torch_gcu::profiler::impl::kineto::activity_t;
 ...
 private:
  ...
  void reassociate() {
    ...
    // relationship between `libkineto_gcu::ITraceActivity` and `Result`.
    ...
    if (results_.get().size() != kineto_events_.size()) {
      TORCH_WARN(
          fmt::format("Failed to recover relationship between all profiler and "
                      "kineto events: "
                      "{} vs. {}  reassociated.",
  ...
  std::shared_ptr<Result> resultFromActivity(const itrace_t* activity) {
    ...
    torch_gcu::profiler::impl::kineto::DeviceAndResource device_and_resource{
  ...
  std::shared_ptr<Result> toResult(const itrace_t* activity) {
    ...
    if (e == nullptr &&
        (type == libkineto_gcu::ActivityType::CPU_OP ||
         type == libkineto_gcu::ActivityType::CPU_INSTANT_EVENT ||
         type == libkineto_gcu::ActivityType::USER_ANNOTATION ||
         type == libkineto_gcu::ActivityType::PYTHON_FUNCTION)) {
      TORCH_WARN_ONCE(
    ...
    for (auto& e : results_.get()) {
      ...
      e->visit(c10::overloaded(
          [&](const ExtraFields<EventType::Kineto>& i) {
            if (i.flow.type == libkineto_gcu::kLinkAsyncCpuGcu &&
                i.flow.start) {
              auto inserted = flow_map.insert({i.flow.id, e});
#ifdef USE_ROCM
              if (inserted.second) {
                TORCH_WARN_ONCE("ROCTracer produced duplicate flow start: ",
                                i.flow.id);
              }
#else   // USE_ROCM
    ...
    for (auto& e : results_.get()) {
      e->visit(c10::overloaded(
          [&](const ExtraFields<EventType::Kineto>& i) {
            ...
            if (it != flow_map.end() &&
                i.flow.type == libkineto_gcu::kLinkAsyncCpuGcu &&
                !i.flow.start) {
              e->parent_ = it->second;
            }
  ...
  for (std::shared_ptr<Result>& r : out) {
    ...
    if (r->parent_.expired()) {
      ...
      min_start_time = adjust_timestamps_dfs(
          r, std::max(r->tag() != EventType::Vulkan
                          ? r->start_time_ns_
...
RecordQueue::getRecords(
  ...
  // ev_start and ev_end
  auto right_intersection_only = [&](ProfilerStepInfo base, int64_t ev_start,
                                     int64_t ev_end) {
  ...
  for (auto& subqueue_it : sub_queues_) {
    ...
    queue.torch_ops_.materialize(out, step_info, converter, queue.tid(),
                                 queue.kineto_info());
  ...
  if (python_tracer_) {
    std::vector<std::shared_ptr<torch_gcu::profiler::impl::Result>> ev;
    try {
      ev = python_tracer_->getEvents(converter, python_enters,
      ...
      // exception.
      torch_gcu::profiler::impl::kineto::stopTrace();
      throw;
    }
...
bool get_fwd_bwd_enabled() { return fwd_bwd_enabled_fn()(); }
...
namespace {
std::function<bool()>& gcu_sync_enabled_fn() {
  static std::function<bool()> fn = []() { return false; };
  return fn;
}
}  // namespace

bool get_gcu_sync_enabled() { return gcu_sync_enabled_fn()(); }

void set_gcu_sync_enabled_fn(std::function<bool()> fn) {
  gcu_sync_enabled_fn() = std::move(fn);
}

void set_gcu_sync_enabled_val(bool val) {
  gcu_sync_enabled_fn() = [val]() { return val; };
}
