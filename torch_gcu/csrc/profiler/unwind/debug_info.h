...
#include <torch_gcu/csrc/profiler/unwind/dwarf_symbolize_enums.h>
#include <torch_gcu/csrc/profiler/unwind/lexer.h>
#include <torch_gcu/csrc/profiler/unwind/sections.h>
#include <torch_gcu/csrc/profiler/unwind/unwind_error.h>
...
struct DebugInfo {
  ...
  uint64_t nextOffset() { return end_ - s_.debug_info.data; }
  std::vector<std::pair<uint64_t, uint64_t>> ranges() {
    if (range_ptr_) {
      ...
      if (range_ptr_->second == DW_FORM_rnglistx) {
        ...
        CheckedLexer L = s_.debug_rnglists.lexer(*rnglists_base_ +
                                                 offset * sec_offset_size_);
 ...
 private:
  CheckedLexer parseHeader(uint64_t offset) {
    ...
    UNWIND_CHECK(version_ == 5 || version_ == 4, "unexpected dwarf version {}",
                 version_);
    ...
    LOG_INFO(
        "compilation unit at offset {:x} with length {:x} and "
        "debug_abbrev_offset {:x}\n",
        offset, length_, debug_abbrev_offset_);
    UNWIND_CHECK(address_size == 8,
  ...
  void parseCompileUnit(CheckedLexer& L) {
    ...
    while (true) {
      ...
      if (attr == DW_AT_low_pc) {
        ...
        UNWIND_CHECK(form == DW_FORM_sec_offset,
                     "unexpected rnglists_base form");
      ...
      } else if (form == DW_FORM_udata || form == DW_FORM_rnglistx ||
  ...
  CheckedLexer findAbbrev(uint64_t offset, uint64_t entry) {
    ...
    while (true) {
      ...
      UNWIND_CHECK(abbrev_code != 0, "could not find entry {} at offset {:x}",
                   entry, offset);
      auto tag = L.readULEB128();
      L.read<uint8_t>();  // has children
      if (abbrev_code == entry) {
        UNWIND_CHECK(tag == DW_TAG_compile_unit,
                     "first entry was not a compile unit but {}", tag);
        return L;
      }
      while (true) {
