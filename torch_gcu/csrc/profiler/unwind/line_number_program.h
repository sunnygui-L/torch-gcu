...
#include <torch_gcu/csrc/profiler/unwind/debug_info.h>
...
struct LineNumberProgram {
  ...
  void parse() {
    ...
    UNWIND_CHECK(version == 5 || version == 4,
                 "expected version 4 or 5 but found {}", version);
    if (version == 5) {
    ...
    UNWIND_CHECK(minimum_instruction_length_ == 1,
 ...
 private:
  ...
  void produceEntry() {
    ...
    PRINT_LINE_TABLE("{:x}\t{}\t{}\n", address_, filename(entry_.file),
                     entry_.line);
    UNWIND_CHECK(entry_.file < file_names_.size(), "file index {} > {} entries",
