...
struct Section {
  char* data = nullptr;
  size_t size = 0;
  const char* string(size_t offset) { return lexer(offset).readCString(); }
...
struct MemFile {
  explicit MemFile(const char* filename_)
    ...
    UNWIND_CHECK(fd_ != -1, "failed to open {}: {}", filename_,
                 strerror(errno));
    // NOLINTNEXTLINE
    struct stat s;
    if (-1 == fstat(fd_, &s)) {
    ...
    n_bytes_ = s.st_size;
    UNWIND_CHECK(n_bytes_ > sizeof(Elf64_Ehdr), "empty shared library: {}",
                 filename_);
...
#undef ELF_CHECK
    ...
    UNWIND_CHECK(ehdr_->e_shstrndx < ehdr_->e_shnum,
                 "invalid strtab section offset");
