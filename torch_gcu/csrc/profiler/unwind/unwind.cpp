...
#include <torch_gcu/csrc/profiler/unwind/communicate.h>
#include <torch_gcu/csrc/profiler/unwind/dwarf_enums.h>
#include <torch_gcu/csrc/profiler/unwind/eh_frame_hdr.h>
#include <torch_gcu/csrc/profiler/unwind/fast_symbolizer.h>
#include <torch_gcu/csrc/profiler/unwind/fde.h>
#include <torch_gcu/csrc/profiler/unwind/unwinder.h>
...
struct LibraryInfo {
  LibraryInfo(std::string name, uint64_t load_bias, uint64_t last_addr,
...
struct UnwindCache {
  Version currentVersion() {
    Version r;
    dl_iterate_phdr(
        [](struct dl_phdr_info* info, size_t size [[maybe_unused]],
           void* data) {
        ...
        [](struct dl_phdr_info* info, size_t size [[maybe_unused]],
          ...
          for (auto i : c10::irange(info->dlpi_phnum)) {
            ...
            if (segments[i].p_type == PT_GNU_EH_FRAME) {
              ...
              self->all_libraries_.emplace_back(std::move(library_name),
                                                info->dlpi_addr, last_addr,
                                                eh_frame_hdr);
              return 0;
            }
          }
  ...
  const Unwinder& unwinderFor(
      uint64_t addr, std::shared_lock<std::shared_timed_mutex>& rdlock) {
  ...
  torch_gcu::unwind::Stats stats() { return stats_; }
...
std::optional<std::pair<std::string, uint64_t>> libraryFor(void* addr) {
  ...
  return std::make_pair(library_info->name(),
                        (uint64_t)addr - library_info->load_bias());
}
...
struct Symbolizer {
 ...
 private:
  ...
  Entry& getOrCreate(const std::string& name) {
    ...
    if (it == entries_.end()) {
      // NOLINTNEXTLINE(*-c-arrays*)
      const char* args[] = {addr2line_binary_, "-C",   "-f", "-e",
                            name.c_str(),      nullptr};
      it = entries_
...
static std::vector<Frame> symbolize_fast(const std::vector<void*>& frames,
                                         Mode mode) {
...
extern "C" void unwind_c(std::vector<void*>* result, int64_t rsp, int64_t rbp) {
  std::shared_lock lock(torch_gcu::unwind::cache_mutex_);
  torch_gcu::unwind::UnwindState state{};
  // NOLINTNEXTLINE(performance-no-int-to-ptr)
  ...
  state.rsp = rsp + 8;
  state.rbp = rbp;
  torch_gcu::unwind::unwind_cache.checkRefresh(lock);
  while (true) {  // unwind for _start sets rip as being undefined
    // NOLINTNEXTLINE(performance-no-int-to-ptr)
    result->push_back((void*)state.rip);
    const torch_gcu::unwind::Unwinder& uw =
        torch_gcu::unwind::unwind_cache.unwinderFor(state.rip, lock);
    if (uw.terminator()) {
      if (uw.isUnknown()) {
