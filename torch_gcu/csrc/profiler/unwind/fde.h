...
#include <torch_gcu/csrc/profiler/unwind/action.h>
...
struct FDE {
  FDE(void* data, const char* library_name, uint64_t load_bias)
    ...
    TORCH_INTERNAL_ASSERT(version == 1 || version == 3,
                          "non-1 version for CIE");
  ...
  void def_cfa_register(int64_t reg) { def_cfa(reg, last_offset_); }
  void def_cfa_offset(int64_t off) { def_cfa(last_reg_, off); }
  ...
  TableState& state() { return state_stack_.back(); }
