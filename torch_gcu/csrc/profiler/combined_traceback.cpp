...
#include <torch_gcu/csrc/profiler/combined_traceback.h>

namespace torch_gcu {

static std::atomic<CapturedTraceback::Python*> python_support_ = nullptr;

std::shared_ptr<CapturedTraceback> CapturedTraceback::gather(bool python,
                                                             bool script,
                                                             bool cpp) {
...
struct PyFrameEq {
  std::size_t operator()(const CapturedTraceback::PyFrame& lhs,
...
SymbolizedTracebacks symbolize(
  ...
  for (const auto& sc : to_symbolize) {
    ...
    auto append_python = [&](const CapturedTraceback::PyFrame& f) {
      ...
      r.tracebacks.back().insert(r.tracebacks.back().end(), fragment.begin(),
                                 fragment.end());
    };
    ...
    for (void* f : sc->cpp_frames_) {
      ...
      } else if (uf.funcname.rfind("torch::jit::InterpreterStateImpl::run",
                                   0) != std::string::npos) {
        append_jit();
      }
