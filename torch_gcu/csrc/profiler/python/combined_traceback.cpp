...
// GCUCachingAllocator may hold its device lock when ~StackContext runs.
...
struct PythonTraceback : public CapturedTraceback::Python {
  ...
  int traverse(std::vector<CapturedTraceback::PyFrame>& frames,
               void_visitproc visit, void* arg) override {
    for (auto& f : frames) {
      Py_VISIT(f.code);
    }
    return 0;
  ...
  void appendSymbolized(
    ...
    for (const auto& f : to_symbolize) {
      ...
      result.all_frames.emplace_back(
          unwind::Frame{py::cast<std::string>(filename),
                        py::cast<std::string>(funcname), (uint64_t)lineno});
      ...
      if (stack_frames_for_code.ptr()) {
        ...
        if (!extra.is_none()) {
          for (py::handle h : extra) {
            result.tracebacks.back().push_back(result.all_frames.size());
            result.all_frames.emplace_back(
                unwind::Frame{py::cast<std::string>(h[filename_s]),
