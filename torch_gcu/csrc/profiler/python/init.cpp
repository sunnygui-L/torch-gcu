...
#include <torch_gcu/csrc/profiler/python/combined_traceback.h>
#include <torch_gcu/csrc/profiler/python/init.h>
#include <torch_gcu/csrc/profiler/standalone/execution_trace_observer.h>

struct THPCapturedTraceback {
  PyObject_HEAD std::shared_ptr<torch_gcu::CapturedTraceback> data;
};

static int THPCapturedTraceback_traverse(PyObject* self, visitproc visit,
                                         void* arg) {
...
static void THPCapturedTraceback_dealloc(PyObject* self_) {
  ...
  self->data.~shared_ptr<torch_gcu::CapturedTraceback>();
  // promptly trigger delayed frees since we have GIL
  torch_gcu::freeDeadCapturedTracebackFrames();
  PyObject_GC_Del(self);
}

PyTypeObject THPCapturedTracebackType = {
    PyVarObject_HEAD_INIT(
        nullptr, 0) "torch._C._profiler.CapturedTraceback", /* tp_name */
...
template <>
struct type_caster<std::shared_ptr<torch_gcu::CapturedTraceback>> {
 public:
  PYBIND11_TYPE_CASTER(std::shared_ptr<torch_gcu::CapturedTraceback>,
  ...
  static handle cast(std::shared_ptr<torch_gcu::CapturedTraceback> src,
                     return_value_policy /* policy */, handle /* parent */) {
    auto* r = PyObject_GC_New(THPCapturedTraceback, &THPCapturedTracebackType);
    new (&r->data)
        std::shared_ptr<torch_gcu::CapturedTraceback>(std::move(src));
...
PyObject* RecordFunctionFast_new(PyTypeObject* subtype, PyObject* args,
                                 PyObject* kwargs) {
...
int RecordFunctionFast_init(PyObject* selfGeneric, PyObject* args,
                            PyObject* kwargs) {
  auto self = (RecordFunctionFast*)selfGeneric;
  // NOLINTNEXTLINE(*-c-arrays*)
  constexpr const char* kwlist[] = {"name", "input_values", "keyword_values",
                                    nullptr};
  ...
  if (!PyArg_ParseTupleAndKeywords(
          ...
          const_cast<char**>(kwlist), &name, &input_values, &keyword_values)) {
    return -1;
  }
  if (name) {
    TORCH_CHECK(THPUtils_checkString(name),
                "The name passed to RecordFunctionFast must be a string");
    Py_INCREF(name);
    self->name = name;
  }
  if (input_values) {
    TORCH_CHECK(PyList_Check(input_values) || PyTuple_Check(input_values),
...
PyObject* RecordFunctionFast_enter(PyObject* selfGeneric, PyObject* unused) {
  HANDLE_TH_ERRORS
  if (torch_gcu::profiler::impl::ProfilerStateBase::get() != nullptr) {
    auto self = (RecordFunctionFast*)selfGeneric;
    TORCH_INTERNAL_ASSERT(!self->guard,
                          "Trying to enter a new record_function_fast context "
                          "but the guard is unexpectedly already set");
    self->guard =
        std::make_unique<at::RecordFunction>(at::RecordScope::FUNCTION);
    std::vector<at::IValue> args;
    std::unordered_map<std::string, at::IValue> kwargs;
    bool profiler_need_input =
        torch_gcu::autograd::profiler::profilerEnabled() &&
        torch_gcu::autograd::profiler::getProfilerConfig().report_input_shapes;
...
PyObject* RecordFunctionFast_exit(PyObject* selfGeneric, PyObject* unused) {
  ...
  if (torch_gcu::profiler::impl::ProfilerStateBase::get() != nullptr) {
    auto self = (RecordFunctionFast*)selfGeneric;
    TORCH_INTERNAL_ASSERT(self->guard,
                          "Trying to exit an active record_function_fast "
                          "context but no guard is set");
    self->guard.reset();
  }
  Py_RETURN_NONE;
  END_HANDLE_TH_ERRORS
...
void initPythonBindings(PyObject* module) {
  ...
  // Import ExperimentalConfig and ProfilerConfig from torch._C._profiler
  // to avoid duplicate registration since we use torch::profiler::impl types
  bool experimental_config_imported = false;
  bool profiler_config_imported = false;
  try {
    auto torch_profiler = py::module::import("torch._C._profiler");
    if (py::hasattr(torch_profiler, "_ExperimentalConfig")) {
      m.attr("_ExperimentalConfig") =
          torch_profiler.attr("_ExperimentalConfig");
      experimental_config_imported = true;
    }
    if (py::hasattr(torch_profiler, "ProfilerConfig")) {
      m.attr("ProfilerConfig") = torch_profiler.attr("ProfilerConfig");
      profiler_config_imported = true;
    }
  } catch (const py::error_already_set&) {
    // If torch._C._profiler doesn't exist or doesn't have these types,
    // we'll register them ourselves. Clear the error state.
    PyErr_Clear();
  }

  // Only register ExperimentalConfig if we couldn't import from torch
  if (!experimental_config_imported) {
    py::class_<ExperimentalConfig>(m, "_ExperimentalConfig")
        .def(
            py::init<std::vector<std::string> /* profiler_metrics */,
                     bool /* profiler_measure_per_kernel */, bool /* verbose */,
            ...
            "       to measure GCU performance events.\n"
            "       If this list contains values Kineto runs in CUPTI profiler "
            "mode\n"
            "    profiler_measure_per_kernel (bool) : whether to profile "
            "metrics per kernel\n"
            "       or for the entire measurement duration.\n"
            "    verbose (bool) : whether the trace file has `Call stack` "
            "field "
            "or not.\n"
            "    performance_events : a list of profiler events to be used for "
            "measurement.\n"
            "    enable_cuda_sync_events : for GCU profiling mode, enable "
            "adding GCU synchronization events\n"
            "       that expose GCU device, stream and event synchronization "
            "activities. This feature is new\n"
        ...
        .def(py::pickle(
            [](const ExperimentalConfig& p) {  // __getstate__
              ...
              return py::make_tuple(py_metrics, p.profiler_measure_per_kernel,
                                    p.verbose, p.enable_cuda_sync_events,
            ...
            [](const py::tuple& t) {  // __setstate__
              ...
              return ExperimentalConfig(
                  ...
                  std::move(performance_events), t[3].cast<bool>());
            }));
  }

  // Only register ProfilerConfig if we couldn't import from torch
  if (!profiler_config_imported) {
    py::class_<ProfilerConfig>(m, "ProfilerConfig")
        .def(py::init<ProfilerState, bool, /* report_input_shapes */
                      bool,                /* profile_memory */
                      bool,                /* with_stack */
                      bool,                /* with_flops */
                      bool,                /* with_modules */
      ...
      .def_property_readonly("parameters",
                             [](const NNModuleInfo& s) {
                               ...
                               for (const auto& p : s.parameters_) {
                                 out.append(py::make_tuple(p.name_, p.metadata_,
                                                           p.grad_metadata_));
                               }
                               return out;
                             })
  ...
  // NOLINTNEXTLINE(bugprone-unused-raii)
  py::class_<ExtraFields<EventType::OutOfMemory>>(m,
  ...
  py::class_<Result, std::shared_ptr<Result>>(m, "_ProfilerEvent")
      ...
      .def_property_readonly("parent",
                             [](const Result& r) { return r.parent_.lock(); })
  ...
  m.def("_add_execution_trace_observer",
        &torch_gcu::profiler::impl::addExecutionTraceObserver,
        py::arg("output_file_name"));
  m.def("_remove_execution_trace_observer",
        &torch_gcu::profiler::impl::removeExecutionTraceObserver);
  m.def("_enable_execution_trace_observer",
        &torch_gcu::profiler::impl::enableExecutionTraceObserver);
  m.def("_disable_execution_trace_observer",
        &torch_gcu::profiler::impl::disableExecutionTraceObserver);
  m.def("_set_record_concrete_inputs_enabled_val",
        &torch_gcu::profiler::impl::set_record_concrete_inputs_enabled_val);
  m.def("_set_fwd_bwd_enabled_val",
        &torch_gcu::profiler::impl::set_fwd_bwd_enabled_val);
  m.def("_set_gcu_sync_enabled_val",
        &torch_gcu::profiler::impl::set_gcu_sync_enabled_val);

  TORCH_CHECK(PyType_Ready(&THPCapturedTracebackType) >= 0);
  PyModule_AddObject(m.ptr(), "CapturedTraceback",
                     (PyObject*)&THPCapturedTracebackType);
  m.def("gather_traceback", CapturedTraceback::gather, py::arg("python") = true,
        py::arg("script") = true, py::arg("cpp") = true);
  ...
  m.def("symbolize_addresses", [](const std::vector<uint64_t>& frames,
                                  const std::string& mode_s) {
    std::vector<std::tuple<std::string, int64_t, std::string>> frames_out;
    torch_gcu::unwind::Mode mode = torch_gcu::unwind::Mode::addr2line;
    if (mode_s == "fast") {
      mode = torch_gcu::unwind::Mode::fast;
    } else if (mode_s == "addr2line") {
      mode = torch_gcu::unwind::Mode::addr2line;
    } else if (mode_s == "dladdr") {
      mode = torch_gcu::unwind::Mode::dladdr;
    } else {
  ...
  if (PyModule_AddObject(m.ptr(), "_RecordFunctionFast",
