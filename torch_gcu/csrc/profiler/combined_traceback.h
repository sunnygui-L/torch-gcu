...
struct TORCH_API CapturedTraceback : public c10::GatheredContext {
  ...
  static std::shared_ptr<CapturedTraceback> gather(bool python, bool script,
                                                   bool cpp);
  ...
  struct Python {
    ...
    virtual void appendSymbolized(const std::vector<PyFrame>& to_symbolize,
                                  SymbolizedTracebacks& st) = 0;
    // tp_traverse/tp_clear implementations
    virtual int traverse(std::vector<PyFrame>& frames, visitproc visit,
                         void* arg) = 0;
