...
#include "torch_gcu/csrc/profiler/orchestration/python_tracer.h"
...
// ============================================================================
static const char* internStringView(std::string_view value) {
  ...
  auto inserted = pool.emplace(std::move(key));
...
struct CodeLocation {
  ...
  explicit CodeLocation(PyFrameObject* frame)
    ...
    filename_ = internStringView(THPUtils_unpackStringView(code->co_filename));
    name_ = internStringView(THPUtils_unpackStringView(code->co_name));
  }

  bool operator==(const CodeLocation& other) const {
    return strcmp(filename_, other.filename_) == 0 &&
...
template <>
struct std::hash<torch_gcu::profiler::impl::CodeLocation> {
  size_t operator()(const torch_gcu::profiler::impl::CodeLocation& x) {
    return c10::get_hash(std::string_view(x.filename_),
                         std::string_view(x.name_), x.line_number_);
  }
};
...
struct Config<CallType::PyModuleCall>
    : ExtendedPyCallConfig<PyModuleSelf, PyModuleCls,
                           NNModuleInfo::ParameterInfo> {};

template <>
struct Config<CallType::PyOptimizerCall>
    : ExtendedPyCallConfig<PyOptimizerSelf, PyOptimizerCls,
...
class Callsite {
 public:
  ...
  static_assert(std::is_trivially_copyable_v<key_t>,
 ...
 public:
  ...
  auto load(const Callsite<C>& callsite, size_t python_tid) const {
    ...
    return ExtraFields<Config<C>::event_type>{
        /*end_time_ns=*/std::numeric_limits<c10::time_t>::min(), python_tid,
...
typename Config<C>::cls_t set_class(
    ValueCache* value_cache, typename Config<C>::cache_t& cache,
...
std::vector<std::pair<std::string, TensorMetadata>> ValueCache::unpackTensorMap(
  ...
  for (auto& it : tensor_map) {
    ...
    if (py::isinstance<py::str>(it.first) && THPVariable_CheckExact(value)) {
      out.emplace_back(py::cast<std::string>(it.first),
                       toTensorMetadata(value));
    }
  }
  return out;
}
  ...
  if (C10_UNLIKELY(cache.cls_and_parameters_.find(key) ==
    ...
    for (auto& it : params) {
      ...
      if (py::isinstance<py::str>(it.first) && THPVariable_CheckExact(p)) {
        params_.push_back(
            {it.first.cast<std::string>(), toTensorMetadata(p),
...
ExtraFields<EventType::PyCall>::args_t ValueCache::load<CallType::PyModuleCall>(
  ...
  NNModuleInfo info{key, cls, cache.cls_names_.at(cls),
                    cls_and_parameters.parameters_};
  return {
...
ValueCache::load<CallType::PyOptimizerCall>(
    const PyOptimizerCallKey& key) const {
  auto& cache = std::get<CallType::PyOptimizerCall>(state_);
  const auto& cls_and_parameters = cache.cls_and_parameters_.at(key);
  auto cls = cls_and_parameters.cls_;
  OptimizerInfo info{key, cls, cache.cls_names_.at(cls),
                     cls_and_parameters.parameters_};
  return {
...
struct TraceKeyCacheState {
  ...
  TraceKey intern(Callsite<C> callsite,
  ...
  auto lookup(Callsite<C>& callsite, ValueCache& value_cache) const {
    return std::make_pair(value_cache.load<C>(callsite.value_),
...
class PythonTracer final : public python_tracer::PythonTracerBase {
 public:
  PythonTracer(torch_gcu::profiler::impl::RecordQueue* queue);
 ...
 private:
  ...
  torch_gcu::profiler::impl::RecordQueue* queue_;
...
PythonTracer::PythonTracer(torch_gcu::profiler::impl::RecordQueue* queue)
    : queue_(queue),
...
class PostProcess {
 public:
  PostProcess(std::function<c10::time_t(c10::approx_time_t)> time_converter,
              std::deque<ThreadLocalResults>& tls,
              const ValueCache& value_cache, c10::time_t end_time_ns)
      : end_time_{end_time_ns}, time_converter_{std::move(time_converter)} {
    for (size_t python_tid : c10::irange(tls.size())) {
      CallTypeHelper<TraceKeyCacheState>::map(tls[python_tid].trace_keys_,
                                              *this, value_cache, python_tid);
  ...
  void set_start_frames(
    ...
    for (const auto& frame : start_frames) {
      enters.push_back({frame.trace_key_,
  ...
  template <CallType C>
  void operator()(const TraceKeyCacheState<C>& trace_cache,
                  const ValueCache& value_cache, size_t python_tid) {
  ...
  void addExits(AppendOnlyList<c10::approx_time_t, N>& exits,
                size_t python_tid) {
  ...
  std::vector<std::shared_ptr<Result>> run(
    ...
    std::stable_sort(
        ...
        [](const auto a, const auto b) { return a.enter_t_ < b.enter_t_; });
 ...
 private:
  ...
  void populate(std::vector<python_tracer::CompressedEvent>& enters,
                std::vector<std::shared_ptr<Result>>& out) {
    ...
    for (const auto& enter : enters) {
      ...
      if (fields_it != state.fields_.end()) {
        ...
        out.push_back(Result::create(enter.enter_t_, enter.system_tid_,
                                     enter.kineto_info_, fields_it->second));
...
std::vector<std::shared_ptr<Result>> PythonTracer::getEvents(
  ...
  PostProcess post_process(std::move(time_converter), thread_local_results_,
                           value_cache_, end_time_ns);
...
std::unique_ptr<python_tracer::PythonTracerBase> getTracer(
    torch_gcu::profiler::impl::RecordQueue* queue) {
...
void init() {
  pybind11::gil_scoped_acquire gil;
  TORCH_CHECK(PyType_Ready(&torch_gcu::profiler::impl::TraceContextType) == 0);
  torch_gcu::profiler::impl::python_tracer::registerTracer(
      &torch_gcu::profiler::impl::getTracer);
}
}  // namespace torch_gcu::autograd::profiler::python_tracer
