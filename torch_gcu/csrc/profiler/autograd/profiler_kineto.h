...
using experimental_event_t = std::shared_ptr<torch_gcu::profiler::impl::Result>;
using extra_meta_t = std::unordered_map<std::string, std::string>;

struct TORCH_API KinetoEvent {
  KinetoEvent(const std::shared_ptr<const torch_gcu::profiler::impl::Result>&,
              const bool verbose);
  ...
  int64_t gcuElapsedUs() const;
  int64_t privateuse1ElapsedUs() const;
  void getPerfEventCounters(torch_gcu::profiler::perf_counters_t&) const;
  extra_meta_t extraMeta() const;

 private:
  torch_gcu::profiler::impl::ProfilerVoidEventStub fallbackStart() const;
  torch_gcu::profiler::impl::ProfilerVoidEventStub fallbackEnd() const;

  std::shared_ptr<const torch_gcu::profiler::impl::Result> result_;
...
TORCH_API void reportBackendEventToActiveKinetoProfiler(
    const int64_t start_time_us, const int64_t end_time_us,
    const int64_t debug_handle, const at::RecordScope scope,
    const std::string& event_name, const std::string& backend_name);
 ...
 * Without calling these functions, the symptom may be "not seeing GCU events
