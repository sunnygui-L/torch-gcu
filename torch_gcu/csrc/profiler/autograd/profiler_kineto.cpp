...
#include "torch_gcu/csrc/profiler/containers.h"
#include "torch_gcu/csrc/profiler/events.h"
...
#include "torch_gcu/csrc/profiler/standalone/privateuse1_observer.h"
...
__attribute__((weak)) int acc_get_device_type() {
  throw std::runtime_error(
      "Dummy implementation of acc_get_device_type is not supposed to be "
      "called!");
}
}  // extern "C"
#endif  // _MSC_VER
...
inline int64_t getTimeNs() {
  return libkineto_gcu::timeSinceEpoch(std::chrono::system_clock::now());
}

using torch_gcu::profiler::impl::ActiveProfilerType;
using torch_gcu::profiler::impl::EventType;
using torch_gcu::profiler::impl::ExtraFields;
using torch_gcu::profiler::impl::get_record_concrete_inputs_enabled;
using torch_gcu::profiler::impl::ivalueListToStr;
using torch_gcu::profiler::impl::ivalueToStr;
using torch_gcu::profiler::impl::op_input_t;
using torch_gcu::profiler::impl::ProfilerStateBase;
using torch_gcu::profiler::impl::PyExtraFieldsBase;
using torch_gcu::profiler::impl::Result;
using torch_gcu::profiler::impl::shape;
using torch_gcu::profiler::impl::shapesToStr;
using torch_gcu::profiler::impl::stacksToStr;
using torch_gcu::profiler::impl::strListToStr;
using torch_gcu::profiler::impl::TensorMetadata;
using torch_gcu::profiler::impl::variantShapesToStr;
...
auto parseArgData(const std::vector<op_input_t>& input_shapes,
  ...
  return OpArgData{
      true,   shapes, dtypes, concrete_inputs_list, shapesForKinetoEvent,
      strides};
}
...
struct MetadataBase {
  /* implicit */ MetadataBase(const std::shared_ptr<Result>& result)
    ...
    if (std::holds_alternative<ExtraFields<EventType::Kineto>>(
      ...
      // `libkineto_gcu::ITraceActivity` to
      // `libkineto_gcu::GenericTraceActivity`. We know that all activities
      // provided by PyTorch are of the correct type, however Kineto profilers
      // can (and do) add events that inherit directly from ITraceActivity. As a
      // result, any Result which was constructed from an event that Kineto
      // provided is unsafe to cast.
  ...
  void addMetadata(const std::string& key, const std::string& value) {
    if (kinetoActivity_ && !value.empty() && value != "\"\"") {
      torch_gcu::profiler::impl::kineto::addMetadata(
          // NOLINTNEXTLINE(cppcoreguidelines-pro-type-const-cast)
          const_cast<torch_gcu::profiler::impl::kineto::activity_t*>(
              kinetoActivity_),
          key, value);
    }
  }
 ...
 private:
  const torch_gcu::profiler::impl::kineto::activity_t* kinetoActivity_{nullptr};
};
...
struct AddGenericMetadata : public MetadataBase {
  AddGenericMetadata(std::shared_ptr<Result>& result,
                     const torch_gcu::profiler::impl::ProfilerConfig* config)
  ...
  void operator()(ExtraFields<EventType::TorchOp>& op_event) {
    ...
    if (arg_data.hasData) {
      ...
      if (!arg_data.concreteInputs.empty()) {
        addMetadata("Concrete Inputs",
                    ivalueListToStr(arg_data.concreteInputs));
      }
    }
    ...
    addMetadata("Record function id",
                std::to_string(op_event.record_function_id_));
  }
 ...
 private:
  ...
  const torch_gcu::profiler::impl::ProfilerConfig* config_;
};
...
struct KinetoThreadLocalState : public ProfilerStateBase {
  ...
  void reportMemoryUsage(void* ptr, int64_t alloc_size, size_t total_allocated,
    ...
    if (config_.profile_memory && !config_.disabled()) {
      recordQueue.getSubqueue()->emplace_allocation_event(
          c10::getApproximateTime(), ptr, alloc_size, total_allocated,
          total_reserved, device.type(), device.index());
    }
  }
  ...
  void reportOutOfMemory(int64_t alloc_size, size_t total_allocated,
    ...
    if (config_.profile_memory && !config_.disabled()) {
      recordQueue.getSubqueue()->emplace_ooms_event(
          c10::getApproximateTime(), alloc_size, total_allocated,
          total_reserved, device.type(), device.index());
    }
  }
  ...
  std::unique_ptr<torch_gcu::profiler::impl::kineto::ActivityTraceWrapper>
  finalizeTrace() {
    ...
    libkineto_gcu::get_time_converter() = converter;
    ...
    kinetoEvents.erase(
        std::remove_if(kinetoEvents.begin(), kinetoEvents.end(),
  ...
  torch_gcu::profiler::impl::RecordQueue recordQueue;
...
void onFunctionExit(const at::RecordFunction& fn,
  ...
  auto* kineto_ctx_ptr =
      static_cast<torch_gcu::profiler::impl::KinetoObserverContext*>(ctx_ptr);
  ...
  if (config.state == ProfilerState::KINETO_PRIVATEUSE1_FALLBACK) {
    try {
      auto fallback = kineto_ctx_ptr->fallback_;
      TORCH_INTERNAL_ASSERT(fallback != nullptr);
      torch_gcu::profiler::impl::privateuse1Stubs()->record(
          nullptr, &fallback->device_event_end_, nullptr);
    } catch (const std::exception& e) {
      LOG(WARNING) << "Failed to record GCU event. " << e.what();
    }
  }

  if (fn.scope() == at::RecordScope::USER_SCOPE) {
    torch_gcu::profiler::impl::kineto::popUserCorrelationId();
  } else {
    torch_gcu::profiler::impl::kineto::popCorrelationId();
  }
}
...
void reportBackendEventToActiveKinetoProfiler(const int64_t start_time_us,
  ...
  state_ptr->recordQueue.getSubqueue()->emplace_backend_event(
      start_time_us, end_time_us, debug_handle, scope, event_name,
      backend_name);
...
void prepareProfiler(
    const torch_gcu::profiler::impl::ProfilerConfig& config,
    const std::set<torch_gcu::profiler::impl::ActivityType>& activities) {
  ...
  torch_gcu::profiler::impl::kineto::prepareTrace(
      /*cpuOnly=*/false, activities, config.experimental_config);

  if (!config.experimental_config.performance_events.empty()) {
    /* For now only CPU activity is supported */
    TORCH_CHECK(
        activities.count(torch_gcu::autograd::profiler::ActivityType::CPU),
        "Cannot run cpu hardware profiler without CPU activities, please only "
        "use CPU activity type");
    /*
    ...
    auto is_standard_event = [](const std::string& event) -> bool {
      for (auto e : torch_gcu::profiler::ProfilerPerfEvents) {
...
void toggleCollectionDynamic(
    const bool enable,
    const std::set<torch_gcu::profiler::impl::ActivityType>& activities) {
  if (activities.count(torch_gcu::autograd::profiler::ActivityType::CPU) > 0 &&
      activities.count(
          torch_gcu::autograd::profiler::ActivityType::PrivateUse1) == 0) {
    LOG(WARNING) << "Toggling CPU activity with GCU activity on may result in "
                    "traces with GCU events on artibrary tracks";
  }
  for (auto act : activities) {
    if (act == torch_gcu::autograd::profiler::ActivityType::PrivateUse1) {
      torch_gcu::profiler::impl::kineto::toggleCollectionDynamic(enable);
    } else if (act == torch_gcu::autograd::profiler::ActivityType::CPU) {
      toggleCPUCollectionDynamic(enable);
    } else {
      LOG(WARNING) << "Dynamic toggle is only supported for CPU/GCU activity, "
...
void enableProfilerWithEventPostProcess(
    const torch_gcu::profiler::impl::ProfilerConfig& config,
    const std::set<torch_gcu::profiler::impl::ActivityType>& activities,
    post_process_t&& cb, const std::unordered_set<at::RecordScope>& scopes) {
  TORCH_CHECK(config.state != ProfilerState::NVTX,
              "NVTX does not support post processing callback.");
  TORCH_CHECK(config.state != ProfilerState::ITT,
...
void enableProfiler(
  ...
  if (config.state == ProfilerState::NVTX) {
    TORCH_WARN("NVTX profiler is not supported on torch_gcu.profiler");
    return;
  } else if (config.state == ProfilerState::ITT) {
    TORCH_WARN("ITT profiler is not supported on torch_gcu.profiler");
    return;
  } else if (config.state == ProfilerState::PRIVATEUSE1) {
    torch_gcu::profiler::impl::pushPRIVATEUSE1CallbacksStub(config, scopes);
    return;
  }
  ...
  if (!config.global()) {
    torch_gcu::profiler::impl::kineto::startTrace();
  }
...
void enableProfilerInChildThread() {
  ...
  TORCH_CHECK(KinetoThreadLocalState::get(/*global=*/false) == nullptr,
...
std::unique_ptr<ProfilerResult> disableProfiler() {
  ...
  // Shared among NVTX, PRIVATEUSE1, KINETO
  ...
  if (config.state == ProfilerState::KINETO ||
    ...
    result = std::make_unique<ProfilerResult>(
        kineto_state_ptr->startTime, std::move(kineto_state_ptr->kinetoEvents),
        std::move(trace), std::move(kineto_state_ptr->eventTree));
  }
...
KinetoEvent::KinetoEvent(
    const std::shared_ptr<const torch_gcu::profiler::impl::Result>& result,
    const bool verbose)
    : result_{result} {
...
bool KinetoEvent::hasShapes() const { return !shapes_.empty(); }

const c10::ArrayRef<std::vector<int64_t>> KinetoEvent::shapes() const {
  return shapes_;
}

bool KinetoEvent::hasTypes() const { return !dtypes_.empty(); }

const c10::ArrayRef<std::string> KinetoEvent::dtypes() const { return dtypes_; }
...
bool KinetoEvent::hasKwinputs() const { return !kwinputs_.empty(); }
...
uint64_t KinetoEvent::endNs() const { return result_->endTimeNS(); }
...
bool KinetoEvent::hasStack() const { return !stack().empty(); }

int64_t KinetoEvent::gcuElapsedUs() const {
  auto gcu_event_start = fallbackStart();
  auto gcu_event_end = fallbackEnd();
  if (!gcu_event_start || !gcu_event_end) {
    return -1;
  }
  try {
    return (int64_t)torch_gcu::profiler::impl::privateuse1Stubs()->elapsed(
        &gcu_event_start, &gcu_event_end);
  } catch (std::exception& e) {
    LOG(WARNING) << "Failed to measure time between two GCU events. "
                 << e.what();
  }
  return -1;
}
...
#define TYPED_ATTR_WITH_DEFAULT(event_type, method_name, expression,   \
                                default_value)                         \
  decltype(std::declval<KinetoEvent>().method_name())                  \
  KinetoEvent::method_name() const {                                   \
...
TYPED_ATTR(TorchOp, flops,
           !e.extra_args_.empty()
               ? torch_gcu::profiler::impl::computeFlops(e.name_, e.extra_args_)
               : 0)
...
ProfilerResult::ProfilerResult(
    uint64_t start_time, std::vector<KinetoEvent> events,
    std::unique_ptr<torch_gcu::profiler::impl::kineto::ActivityTraceWrapper>&&
        trace,
...
void ProfilerResult::save(const std::string& path) { trace_->save(path); }
