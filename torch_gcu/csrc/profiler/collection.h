...
#include <torch_gcu/csrc/profiler/stubs/base.h>
...
struct TORCH_API TensorMetadata : public RawTensorMetadataBase {
  TensorMetadata(const RawTensorMetadata& r, std::vector<int64_t> sizes,
...
using op_input_t = std::variant<TensorMetadata, std::vector<TensorMetadata>,
                                c10::IValue, std::nullopt_t>;
...
struct ExtraFields<EventType::TorchOp> : TorchOpBasicFields {
  ExtraFields(TorchOpBasicFields&& f, uint64_t correlation_id,
              c10::time_t end_time_ns, std::vector<op_input_t>&& inputs,
              std::vector<op_input_t>&& concrete_inputs,
              jit_stack_t&& jit_stack, jit_modules_t&& jit_modules,
              extra_args_t&& extra_args, extra_meta_t&& extra_meta,
              kwinputs_t&& kwinputs, FallbackPair&& device_fallback,
...
// For performance.
static_assert(std::is_pod_v<RawAllocation>, "Non-POD member of RawAllocation.");

template <>
struct ExtraFields<EventType::Allocation> : RawAllocation {
  ExtraFields(const RawAllocation& allocation) : RawAllocation(allocation) {}

  c10::Device device() const { return {device_type_, device_index_}; }
...
// For performance.
static_assert(std::is_pod_v<ExtraFields<EventType::OutOfMemory>>,
...
using strong_t = strong::type<T, Tag, strong::regular,
                              strong::convertible_to<T>, strong::hashable>;
...
struct PyExtraFieldsBase {
  PyExtraFieldsBase(c10::time_t end_time_ns, size_t python_tid,
                    PyFrameState caller)
      : end_time_ns_{end_time_ns},
...
struct ExtraFields<EventType::PyCall> : public PyExtraFieldsBase {
  ...
  ExtraFields(c10::time_t end_time_ns, size_t python_tid, PyFrameState caller,
              args_t args)
...
struct ExtraFields<EventType::Kineto> {
  // Mirrors `libkineto_gcu::GenericTraceActivity::Flow`. This information is
  // used during post processing to properly embed Kineto events into the
  // broader profiler tree structure. End users are not generally expected to
  // use these fields directly, but they are available for debugging.
  struct Flow {
    uint32_t id{0};
    uint32_t type{0};
    uint32_t start{0};
  ...
  libkineto_gcu::ActivityType activity_type_;
  Flow flow;
...
struct TORCH_API Result : public std::enable_shared_from_this<Result> {
  ...
  libkineto_gcu::ActivityType kinetoType() const;
  ...
  std::variant<
      ExtraFields<EventType::TorchOp>, ExtraFields<EventType::Backend>,
      ExtraFields<EventType::Vulkan>, ExtraFields<EventType::Allocation>,
      ExtraFields<EventType::OutOfMemory>, ExtraFields<EventType::PyCall>,
      ExtraFields<EventType::PyCCall>, ExtraFields<EventType::Kineto>>
      extra_fields_;
  ...
  const torch_gcu::profiler::impl::kineto::activity_t* kineto_activity_{
      nullptr};

 private:
  template <EventType E>
  Result(int64_t start_time_ns, uint64_t start_tid,
         kineto::DeviceAndResource kineto_info, ExtraFields<E>&& extra_fields)
...
using perf_profiler_t = torch_gcu::profiler::impl::linux_perf::PerfProfiler;
...
class TORCH_API ThreadLocalSubqueue {
 ...
 private:
  ...
  // with_stack (Python)
  AppendOnlyList<std::pair<python_tracer::TraceKey, c10::approx_time_t>,
                 BlockSize>
      py_calls_;
};
...
class TORCH_API RecordQueue {
 public:
  ...
  getRecords(std::function<c10::time_t(c10::approx_time_t)> time_converter,
...
TORCH_API bool get_gcu_sync_enabled();
TORCH_API void set_gcu_sync_enabled_fn(std::function<bool()>);
TORCH_API void set_gcu_sync_enabled_val(bool);
