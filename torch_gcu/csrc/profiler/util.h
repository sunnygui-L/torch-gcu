...
#define SOFT_ASSERT(cond, ...)                                        \
  [&]() -> bool {                                                     \
    if (C10_UNLIKELY(!(cond))) {                                      \
      torch_gcu::profiler::impl::logSoftAssert(                       \
          __func__, __FILE__, static_cast<uint32_t>(__LINE__), #cond, \
          ::c10::str(__VA_ARGS__));                                   \
      if (torch_gcu::profiler::impl::softAssertRaises()) {            \
...
TORCH_API void logSoftAssert(const char* func, const char* file, uint32_t line,
                             const char* cond, const char* args);
TORCH_API inline void logSoftAssert(
    const char* func, const char* file, uint32_t line, const char* cond,
...
TORCH_API std::string stacksToStr(const std::vector<std::string>& stacks,
                                  const char* delim);
TORCH_API std::vector<std::vector<int64_t>> inputSizes(
    const at::RecordFunction& fn, const bool flatten_list_enabled = false);
...
uint64_t TORCH_API
computeFlops(const std::string& op_name,
