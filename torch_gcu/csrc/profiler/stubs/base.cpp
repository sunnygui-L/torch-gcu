...
struct DefaultStubs : public ProfilerStubs {
  ...
  void record(c10::DeviceIndex*, ProfilerVoidEventStub*,
              int64_t*) const override {
    fail();
  }
  float elapsed(const ProfilerVoidEventStub*,
                const ProfilerVoidEventStub*) const override {
    fail();
    return 0.f;
  }
  void mark(const char*) const override { fail(); }
  void rangePush(const char*) const override { fail(); }
  void rangePop() const override { fail(); }
  bool enabled() const override { return false; }
  void onEachDevice(std::function<void(int)>) const override { fail(); }
  void synchronize() const override { fail(); }
  ~DefaultStubs() override = default;

 private:
  void fail() const { AT_ERROR(name_, " used in profiler but not enabled."); }
...
#define REGISTER_DEFAULT(name, upper_name)                                   \
  ...
  inline const ProfilerStubs*& name##_stubs() {                              \
    ...
    return stubs_;                                                           \
                                                                             ...
                                                                             \
  const ProfilerStubs* name##Stubs() { return name##_stubs(); }              \
                                                                             \
