...
bool setThreadName(const std::string& name) {
  ...
  if (truncated_name.length() > kMaxThreadNameLength) {
    truncated_name = truncated_name.substr(0, kMaxThreadNameLength);
  }
  return 0 == pthread_setname_np(pthread_self(), truncated_name.c_str());
#endif
}
