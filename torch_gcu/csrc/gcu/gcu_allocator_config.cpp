...
namespace torch_gcu::GCUCachingAllocator {

constexpr size_t kRoundUpPowerOfTwoIntervals = 16;

GCUAllocatorConfig::GCUAllocatorConfig()
    : m_max_split_size(std::numeric_limits<size_t>::max()),
      m_garbage_collection_threshold(0),
      m_pinned_num_register_threads(1),
      m_expandable_segments(false),
      m_release_lock_on_topsmalloc(false),
      m_pinned_use_gcu_host_register(false),
      m_last_allocator_settings("") {
  m_roundup_power2_divisions.assign(kRoundUpPowerOfTwoIntervals, 0);
}

size_t GCUAllocatorConfig::roundup_power2_divisions(size_t size) {
...
void GCUAllocatorConfig::lexArgs(const char* env,
...
void GCUAllocatorConfig::consumeToken(const std::vector<std::string>& config,
...
size_t GCUAllocatorConfig::parseMaxSplitSize(
...
size_t GCUAllocatorConfig::parseGarbageCollectionThreshold(
    const std::vector<std::string>& config, size_t i) {
  consumeToken(config, ++i, ':');
  if (++i < config.size()) {
...
size_t GCUAllocatorConfig::parseRoundUpPower2Divisions(
    const std::vector<std::string>& config, size_t i) {
  consumeToken(config, ++i, ':');
...
size_t GCUAllocatorConfig::parseAllocatorConfig(
    const std::vector<std::string>& config, size_t i,
    bool& used_topsMallocAsync) {
  consumeToken(config, ++i, ':');
  if (++i < config.size()) {
    TORCH_CHECK(((config[i] == "native") || (config[i] == "topsMallocAsync")),
                "Unknown allocator backend, "
                "options are native and topsMallocAsync");
    used_topsMallocAsync = (config[i] == "topsMallocAsync");
    TORCH_INTERNAL_ASSERT(config[i] == get()->name(),
...
void GCUAllocatorConfig::parseArgs(const char* env) {
  // If empty, set the default values
  m_max_split_size = std::numeric_limits<size_t>::max();
  m_roundup_power2_divisions.assign(kRoundUpPowerOfTwoIntervals, 0);
  m_garbage_collection_threshold = 0;
  bool used_topsMallocAsync = false;
  ...
  for (size_t i = 0; i < config.size(); i++) {
    if (config[i].compare("max_split_size_mb") == 0) {
      ...
      i = parseAllocatorConfig(config, i, used_topsMallocAsync);
    ...
    } else if (config[i].compare("release_lock_on_topsmalloc") == 0) {
      used_native_specific_option = true;
      consumeToken(config, ++i, ':');
      ++i;
      TORCH_CHECK(
          ...
          "release_lock_on_topsmalloc");
      m_release_lock_on_topsmalloc = (config[i] == "True");
    } else if (config[i].compare("pinned_use_gcu_host_register") == 0) {
      i = parsePinnedUseGcuHostRegister(config, i);
      used_native_specific_option = true;
  ...
  if (used_topsMallocAsync && used_native_specific_option) {
    TORCH_WARN(
        "backend:topsMallocAsync ignores max_split_size_mb,"
        "roundup_power2_divisions, and garbage_collect_threshold.");
  }
}

size_t GCUAllocatorConfig::parsePinnedUseGcuHostRegister(
    const std::vector<std::string>& config, size_t i) {
  consumeToken(config, ++i, ':');
  if (++i < config.size()) {
    TORCH_CHECK((config[i] == "True" || config[i] == "False"),
                "Expected a single True/False argument for "
                "pinned_use_gcu_host_register");
    m_pinned_use_gcu_host_register = (config[i] == "True");
  } else {
    TORCH_CHECK(false, "Error, expecting pinned_use_gcu_host_register value",
                "");
  }
  return i;
}

size_t GCUAllocatorConfig::parsePinnedNumRegisterThreads(
    const std::vector<std::string>& config, size_t i) {
  consumeToken(config, ++i, ':');
  if (++i < config.size()) {
    ...
    auto maxThreads = GCUAllocatorConfig::pinned_max_register_threads();
    TORCH_CHECK(val2 <= maxThreads,
...
void setAllocatorSettings(const std::string& env) {
  std::string current_backend = GCUCachingAllocator::get()->name();
  GCUCachingAllocator::GCUAllocatorConfig::instance().parseArgs(env.c_str());
  std::string new_backend = GCUCachingAllocator::get()->name();
  if (current_backend != new_backend) {
    TORCH_WARN("[GCU Allocator] Allocator backend switched: config_string=\"",
               env, "\", current_backend=", current_backend, " -> ",
               new_backend);
  } else {
    TORCH_WARN("[GCU Allocator] Allocator config updated, backend unchanged: ",
               current_backend);
  }
}
