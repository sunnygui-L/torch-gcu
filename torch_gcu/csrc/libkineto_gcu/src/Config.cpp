...
constexpr int kDefaultActivitiesMaxGcuBufferSize(128 * 1024 * 1024);
...
constexpr int kDefaultMaxEventProfilersPerGcu(1);
...
// 3200000 is the default value set by TOPSPTI
constexpr size_t kDefaultTopsptiDeviceBufferSize(3200000);
// Default value set by TOPSPTI is 250
constexpr size_t kDefaultTopsptiDeviceBufferPoolLimit(20);
...
constexpr char kMaxEventProfilersPerGcuKey[] = "MAX_EVENT_PROFILERS_PER_GCU";
...
constexpr char kActivitiesMaxGcuBufferSizeKey[] =
    "ACTIVITIES_MAX_GCU_BUFFER_SIZE_MB";
constexpr char kActivitiesDisplayGcuSyncWaitEvents[] =
    "ACTIVITIES_DISPLAY_GCU_SYNC_WAIT_EVENTS";
...
// When triggered in this way, /tmp/libkineto_gcu.conf will be used as config.
...
void Config::addConfigFactory(std::string name,
                              std::function<AbstractConfig*(Config&)> factory) {
...
static string defaultTraceFileName() {
  return fmt::format("/tmp/libkineto_activities_{}.json",
                     torch_gcu::util::processId());
}
...
Config::Config()
    : verboseLogLevel_(-1),
      ...
      eventProfilerMaxInstancesPerGcu_(kDefaultMaxEventProfilersPerGcu),
      ...
      activitiesMaxGcuBufferSize_(kDefaultActivitiesMaxGcuBufferSize),
      activitiesWarmupDuration_(kDefaultActivitiesWarmupDurationSecs),
      activitiesWarmupIterations_(0),
      activitiesGcuSyncWaitEvents_(true),
      ...
      topsptiDeviceBufferSize_(kDefaultTopsptiDeviceBufferSize),
      topsptiDeviceBufferPoolLimit_(kDefaultTopsptiDeviceBufferPoolLimit) {
  auto factories = configFactories();
  if (factories) {
...
#endif
  return fmt::format("{:%H:%M:%S}", tm_local);
}
...
bool Config::handleOption(const std::string& name, std::string& val) {
  ...
  } else if (!name.compare(kMaxEventProfilersPerGcuKey)) {
    eventProfilerMaxInstancesPerGcu_ = toInt32(val);
  ...
  // Activity Profiler
    ...
    if (jidx != std::string::npos) {
      activitiesLogUrl_.replace(
          jidx, 14,
          fmt::format("_{}.pt.trace.json", torch_gcu::util::processId()));
    } else {
      jidx = activitiesLogUrl_.find(".json");
      if (jidx != std::string::npos) {
        activitiesLogUrl_.replace(
            jidx, 5, fmt::format("_{}.json", torch_gcu::util::processId()));
      }
    }
    activitiesOnDemandTimestamp_ = timestamp();
  } else if (!name.compare(kActivitiesMaxGcuBufferSizeKey)) {
    activitiesMaxGcuBufferSize_ = toInt32(val) * 1024 * 1024;
  ...
  } else if (!name.compare(kActivitiesDisplayGcuSyncWaitEvents)) {
    activitiesGcuSyncWaitEvents_ = toBool(val);
...
void Config::validate(
  ...
  if (!hasProfileStartTime()) {
    VLOG(0) << "No explicit timestamp has been set. "
            << "Defaulting it to now + activitiesWarmupDuration with a buffer "
               "of double the period of the monitoring thread.";
...
void Config::printActivityProfilerConfig(std::ostream& s) const {
  ...
  if (hasProfileStartIteration()) {
    ...
    s << "  Trace start time: "
      << fmt::format("{:%Y-%m-%d %H:%M:%S}", tm_local);
  ...
  s << "  Max GCU buffer size: " << activitiesMaxGcuBufferSize() / 1024 / 1024
    << "MB" << std::endl;
