...
class TopsptiActivityProfiler {
 public:
  TopsptiActivityProfiler(TopsptiActivityApi& topspti, bool cpuOnly);
  TopsptiActivityProfiler(const TopsptiActivityProfiler&) = delete;
  TopsptiActivityProfiler& operator=(const TopsptiActivityProfiler&) = delete;
  ...
  // memory usage limit (ACTIVITIES_MAX_GCU_BUFFER_SIZE_MB) during warmup.
  ...
  inline void setGcuActivityPresent(bool val) { gcuActivityPresent_ = val; }

  inline bool GcuActivityPresent() { return gcuActivityPresent_; }

  inline bool traceNonEmpty() {
    return cpuActivityPresent_ || gcuActivityPresent_;
  }
  ...
  // Toggle GCU tracing during a profile instance
  ...
  inline void recordThreadInfo() {
    int32_t sysTid = torch_gcu::util::systemThreadId();
    // Note we're using the lower 32 bits of the (opaque) pthread id
    // as key, because that's what TOPSPTI records.
    int32_t tid = torch_gcu::util::threadId();
    ...
    if (resourceInfo_.find({pid, tid}) == resourceInfo_.end()) {
      resourceInfo_.emplace(
          ...
          ResourceInfo(pid, sysTid,
                       sysTid,  // sortindex
                       fmt::format("thread {} ({})", sysTid,
                                   torch_gcu::util::getThreadName())));
    }
  }
 ...
 protected:
  using CpuGcuSpanPair = std::pair<TraceSpan, TraceSpan>;
  static const CpuGcuSpanPair& defaultTraceSpan();

 private:
  // Deferred logging of GCU-event synchronization
  ...
  // Map of gcu activities to user defined events
  class GcuUserEventMap {
   public:
    // Insert a user defined event which maps to the gcu trace activity.
    // If the user defined event mapping already exists this will update the
    // gcu side span to include the span of gcuTraceActivity.
    void insertOrExtendEvent(const ITraceActivity& cpuTraceActivity,
                             const ITraceActivity& gcuTraceActivity);
  ...
  GcuUserEventMap gcuUserEventMap_;
  // id -> activity*
  std::unordered_map<int64_t, const ITraceActivity*> activityMap_;
  // ctops runtime id -> pytorch op id
  // TOPSPTI provides a mechanism for correlating Gcu events to arbitrary
  // external events, e.g.operator activities from PyTorch.
  std::unordered_map<int64_t, int64_t> cpuCorrelationMap_;
  // GCU runtime <-> GCU Activity
  std::unordered_map<int64_t, const ITraceActivity*> correlatedGcuActivities_;
  std::unordered_map<int64_t, int64_t> userCorrelationMap_;

  // data structure to collect topsptiActivityFlushAll() latency overhead
  ...
  void processCpuTrace(libkineto_gcu::CpuTraceBuffer& cpuTrace,
  ...
  // Also creates a corresponding GCU-side span.
  CpuGcuSpanPair& recordTraceSpan(TraceSpan& span, int gcuOpCount);

  // Returns true if net name is to be tracked for a specified number of
  // iterations.
  bool iterationTargetMatch(libkineto_gcu::CpuTraceBuffer& trace);
  ...
  void updateGcuNetSpan(const ITraceActivity& gcuOp);
  bool outOfRange(const ITraceActivity& act);
  void handleGcuActivity(const ITraceActivity& act, ActivityLogger* logger);

  // Process generic TOPSPTI activity
  void handleTopsptiActivity(const Topspti_Activity* record,
                             ActivityLogger* logger);
  // Process specific GCU activity types
  //   void handleCorrelationActivity(
  //       const Topspti_ActivityExternalCorrelation* correlation);
  void handleRuntimeActivity(const Topspti_ActivityAPI* activity,
                             ActivityLogger* logger);
  void handleDriverActivity(const Topspti_ActivityAPI* activity,
                            ActivityLogger* logger);
  //   void handleOverheadActivity(
  //       const Topspti_ActivityOverhead* activity, ActivityLogger* logger);
  //   void handleGcuEventActivity(const Topspti_ActivityGcuEvent* activity);
  //   void handleGcuSyncActivity(
  //       const Topspti_ActivitySynchronization* activity, ActivityLogger*
  //       logger);
  template <class T>
  void handleGcuActivity(const T* act, ActivityLogger* logger);
  ...
  // Calls to TOPSPTI is encapsulated behind this interface
  TopsptiActivityApi& topspti_;
  ...
  // All recorded trace spans, both CPU and GCU
  ...
  bool gcuActivityPresent_{false};
  ...
  // mock tests. All GCU events before this time will be removed
  int64_t captureWindowStartTime_{0};
  // Similarly, all GCU API events after the last net event will be removed
  ...
  struct ErrorCounts {
    ...
    int32_t gcu_and_cpu_op_out_of_order = 0;
    int32_t blocklisted_runtime_events = 0;
    int32_t unexpected_gcu_events = 0;
    bool topspti_stopped_early = false;
  };

  friend std::ostream& operator<<(std::ostream& oss, const ErrorCounts& ecs);

  // This set tracks the (device, gcu streams) observed in the trace
  // doing GCU kernels/memcopies. This prevents emitting GCU sync
