...
#define LOG_IS_ON(severity) (severity >= libkineto_gcu::Logger::severityLevel())

#define LOG_IF(severity, condition)     \
  !(LOG_IS_ON(severity) && (condition)) \
      ? (void)0                         \
      : libkineto_gcu::VoidLogger() &   \
            libkineto_gcu::Logger(severity, __LINE__, __FILE__).stream()
...
#define FILENAME_HASH                          \
  __to_constant__<libkineto_gcu::Logger::hash( \
      libkineto_gcu::Logger::basename(__FILE__))>::val
#define VLOG_IS_ON(verbosity)                                      \
  (libkineto_gcu::Logger::verboseLogLevel() >= verbosity &&        \
   (libkineto_gcu::Logger::verboseLogModules() & FILENAME_HASH) == \
       FILENAME_HASH)
...
#define PLOG(severity) \
  libkineto_gcu::Logger(severity, __LINE__, __FILE__, errno).stream()

#define SET_LOG_SEVERITY_LEVEL(level) \
  libkineto_gcu::Logger::setSeverityLevel(level)

#define SET_LOG_VERBOSITY_LEVEL(level, modules)     \
  libkineto_gcu::Logger::setVerboseLogLevel(level); \
  libkineto_gcu::Logger::setVerboseLogModules(modules)

// Logging the set of devices the trace is collect on.
#define LOGGER_OBSERVER_ADD_DEVICE(device_count) \
  libkineto_gcu::Logger::addLoggerObserverDevice(device_count)

// Incrementing the number of events collected by this trace.
#define LOGGER_OBSERVER_ADD_EVENT_COUNT(count) \
  libkineto_gcu::Logger::addLoggerObserverEventCount(count)

// Record duration of trace in milliseconds.
#define LOGGER_OBSERVER_SET_TRACE_DURATION_MS(duration) \
  libkineto_gcu::Logger::setLoggerObserverTraceDurationMS(duration)

// Record the trace id when given.
#define LOGGER_OBSERVER_SET_TRACE_ID(tid) \
  libkineto_gcu::Logger::setLoggerObserverTraceID(tid)

// Record the group trace id when given.
#define LOGGER_OBSERVER_SET_GROUP_TRACE_ID(gtid) \
  libkineto_gcu::Logger::setLoggerObserverGroupTraceID(gtid)

// Log the set of destinations the trace is sent to.
#define LOGGER_OBSERVER_ADD_DESTINATION(dest) \
  libkineto_gcu::Logger::addLoggerObserverDestination(dest)

// Record this was triggered by On-Demand.
#define LOGGER_OBSERVER_SET_TRIGGER_ON_DEMAND() \
  libkineto_gcu::Logger::setLoggerObserverOnDemand()

// Record this was triggered by On-Demand.
#define LOGGER_OBSERVER_ADD_METADATA(key, value) \
  libkineto_gcu::Logger::addLoggerObserverAddMetadata(key, value)

// UST Logger Semantics to describe when a stage is complete.
#define UST_LOGGER_MARK_COMPLETED(stage) \
  LOG(libkineto_gcu::LoggerOutputType::STAGE) << "Completed Stage: " << stage
