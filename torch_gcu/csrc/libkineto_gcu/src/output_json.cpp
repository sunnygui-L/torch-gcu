...
// TODO: share the same string across c10d, profiler and libkineto_gcu
static constexpr const char* kParamCommsCallName = "record_param_comms";
// Collective function metadata populated from CPU op to GCU kernel
...
static std::string defaultFileName() {
  return fmt::format(kDefaultLogFileFmt, torch_gcu::util::processId());
}
...
void ChromeTraceLogger::handleGenericInstantEvent(
    const libkineto_gcu::ITraceActivity& op) {
  if (!traceOf_) {
    return;
  }

  uint64_t ts = transToRelativeTime(op.timestamp());
  std::string instant_name = op.name();
  std::string instant_meta = op.metadataJson();
  traceOf_ << fmt::format(R"JSON(
  {{
    "ph": "i", "cat": "{}", "s": "t", "name": "{}",
    "pid": {}, "tid": {},
    ...
    "args": {{
      {}
                          ...
                          toString(op.type()), instant_name, op.deviceId(),
                          op.resourceId(), ts / 1000, ts % 1000, instant_meta);
}
...
void ChromeTraceLogger::handleActivity(
  ...
  if (op.type() == ActivityType::GCU_USER_ANNOTATION) {
    // The GCU user annotations start at the same time as the
    // first associated GCU op. Since they appear later
  ...
  // Populate NCCL collective metadata from CPU to GCU
  if (op.type() == ActivityType::CONCURRENT_KERNEL && op.linkedActivity() &&
      op.linkedActivity()->name() == ::libkineto_gcu::kParamCommsCallName) {
    ...
    if (!collectiveName.empty() && !inMsgSize.empty() && !outMsgSize.empty() &&
      ...
      arg_values.append(fmt::format(
          " \"{}\": {}, \"{}\": {}, \"{}\": {}, \"{}\": {}, \"{}\": {}",
          kCollectiveName, collectiveName, kInMsgNelems, inMsgSize,
          kOutMsgNelems, outMsgSize, kGroupSize, groupSize, kDtype, dtype));
    }
    ...
    if (!inSplitSize.empty() && !outSplitSize.empty()) {
      ...
      arg_values.append(fmt::format(" \"{}\": {}, \"{}\": {}", kInSplit,
                                    inSplitSize, kOutSplit, outSplitSize));
    }
    ...
    pg_config.backend_config = "gcu:eccl";
...
void ChromeTraceLogger::handleGenericActivity(
    const libkineto_gcu::GenericTraceActivity& op) {
  handleActivity(op);
}
...
void ChromeTraceLogger::handleGenericLink(const ITraceActivity& act) {
  static struct {
    int type;
    char name[16];
  } flow_names[] = {{kLinkFwdBwd, "fwdbwd"}, {kLinkAsyncCpuGcu, "ac2g"}};
...
void ChromeTraceLogger::addOnDemandDistMetadata() {
  ...
  "distributedInfo": {{"backend": "{}", "rank": {}, "world_size": {}, "pg_count": {}, "pg_config": [)JSON",
                          distInfo_.backend, distInfo_.rank,
                          distInfo_.world_size, std::to_string(pgMap.size()));

  for (const auto& element : pgMap) {
    traceOf_ << fmt::format(
        R"JSON({{"pg_name": {}, "pg_desc": {}, "backend_config": "{}", "pg_size": {}, "ranks": {}}},)JSON",
        element.second.pg_name, element.second.pg_desc,
        element.second.backend_config, element.second.pg_size,
        element.second.ranks);
  }
  traceOf_.seekp(-1, std::ios_base::end);
  traceOf_ << fmt::format(R"JSON(], "nccl_version": "{}"}},)JSON",
