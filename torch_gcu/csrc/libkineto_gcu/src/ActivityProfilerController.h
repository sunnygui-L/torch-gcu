...
class ActivityProfilerController : public ConfigLoader::ConfigHandler {
 public:
  ...
  void pushCorrelationId(uint64_t id) { profiler_->pushCorrelationId(id); }
  void popCorrelationId() { profiler_->popCorrelationId(); }

  void pushUserCorrelationId(uint64_t id) {
    profiler_->pushUserCorrelationId(id);
  }
  void popUserCorrelationId() { profiler_->popUserCorrelationId(); }
 ...
 private:
  ...
  std::unique_ptr<TopsptiActivityProfiler> profiler_;
