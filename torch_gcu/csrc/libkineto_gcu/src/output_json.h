...
class ChromeTraceLogger : public libkineto_gcu::ActivityLogger {
 public:
  ...
  void finalizeTrace(const Config& config,
                     std::unique_ptr<ActivityBuffers> buffers, int64_t endTime,
                     std::unordered_map<std::string, std::vector<std::string>>&
                         metadata) override;

  std::string traceFileName() const { return fileName_; }
 ...
 private:
  ...
  void handleLink(char type, const ITraceActivity& e, int64_t id,
...
class ChromeTraceBaseTime {
 public:
  ...
  int64_t get() {
    ...
    static int64_t base_time = libkineto_gcu::timeSinceEpoch(
