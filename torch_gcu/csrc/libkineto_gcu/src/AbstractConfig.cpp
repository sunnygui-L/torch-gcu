...
int64_t AbstractConfig::toIntRange(const string& val, int64_t min,
                                   int64_t max) const {
  char* invalid;
...
bool AbstractConfig::toBool(string& val) const {
  const std::array<string, 8> bool_vals{"n", "y", "no",    "yes",
                                        "f", "t", "false", "true"};
...
bool AbstractConfig::handleOption(const std::string& /* unused */,
                                  std::string& /* unused */) {
  // Unimplemented
  return false;
}
