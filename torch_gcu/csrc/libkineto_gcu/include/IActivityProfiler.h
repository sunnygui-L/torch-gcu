...
enum class TORCH_GCU_API TraceStatus {
 ...
 *   processes and gcu rows in the trace viewer.
 */
struct TORCH_GCU_API DeviceInfo {
  DeviceInfo(int64_t id, int64_t sortIndex, const std::string& name,
 ...
 */
struct TORCH_GCU_API ResourceInfo {
 ...
 */
class TORCH_GCU_API IActivityProfilerSession {
 public:
 ...
 */
class TORCH_GCU_API IActivityProfiler {
 public:
  ...
  virtual std::unique_ptr<IActivityProfilerSession> configure(
      const std::set<ActivityType>& activity_types, const Config& config) = 0;
