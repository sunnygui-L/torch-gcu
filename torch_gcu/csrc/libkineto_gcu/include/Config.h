...
class TORCH_GCU_API Config : public AbstractConfig {
 public:
  Config();
  ...
  int maxEventProfilersPerGcu() const {
    return eventProfilerMaxInstancesPerGcu_;
  }

  // On Tops we've seen occasional hangs when reprogramming counters
  ...
  int activitiesMaxGcuBufferSize() const { return activitiesMaxGcuBufferSize_; }
  ...
  // Show GCU Synchronization Stream Wait Events
  bool activitiesGcuSyncWaitEvents() const {
    return activitiesGcuSyncWaitEvents_;
  }

  void setActivitiesGcuSyncWaitEvents(bool enable) {
    activitiesGcuSyncWaitEvents_ = enable;
  }
  ...
  static std::chrono::milliseconds alignUp(
      std::chrono::milliseconds duration, std::chrono::milliseconds alignment) {
    duration += alignment;
  ...
  size_t topsptiDeviceBufferSize() const { return topsptiDeviceBufferSize_; }

  size_t topsptiDeviceBufferPoolLimit() const {
    return topsptiDeviceBufferPoolLimit_;
  }
  ...
  static void addConfigFactory(std::string name,
 ...
 private:
  ...
  int eventProfilerMaxInstancesPerGcu_;
  ...
  // Only profile nets with at least this many GCU operators
  int activitiesExternalAPIGcuOpCountThreshold_;
  ...
  // TOPSPTI Device Buffer
  size_t topsptiDeviceBufferSize_;
  size_t topsptiDeviceBufferPoolLimit_;

  // TOPSPTI Timestamp Format
