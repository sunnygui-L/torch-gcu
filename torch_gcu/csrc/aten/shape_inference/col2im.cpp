...
inline void col2im_shape_check(const at::Tensor& input,
  ...
  TORCH_CHECK(
      ...
      kernel_height, " kernel_width: ", kernel_width);
  TORCH_CHECK(stride_width > 0 && stride_height > 0,
              "stride should be greater than zero, but got stride_height: ",
              stride_height, " stride_width: ", stride_width);
  TORCH_CHECK(dilation_width > 0 && dilation_height > 0,
              "dilation should be greater than zero, but got dilation_height: ",
              dilation_height, " dilation_width: ", dilation_width);
  TORCH_CHECK(pad_width >= 0 && pad_height >= 0,
              "padding should be non-negative, but got pad_height: ",
              pad_height, " pad_width: ", pad_width);

  int64_t ndim = input.ndimension();
  // allow dim=0 only the batch dimension.
  TORCH_CHECK((ndim == 2 && input.size(0) != 0 && input.size(1) != 0) ||
  ...
  if (n_input_plane % (kernel_width * kernel_height) != 0) {
    TORCH_CHECK(false,
                ...
                n_input_plane, " and kernel_size=(", kernel_height, ", ",
                kernel_width, ").");
  }

  int64_t input_length = input.size(batch_dim + 2);
  int64_t n_blocks_height =
      div_rtn<int64_t>(output_height + 2 * pad_height -
                           dilation_height * (kernel_height - 1) - 1,
                       stride_height) +
      1;
  int64_t n_blocks_width =
      div_rtn<int64_t>(output_width + 2 * pad_width -
                           dilation_width * (kernel_width - 1) - 1,
                       stride_width) +
      1;

  if (input_length != (n_blocks_height * n_blocks_width)) {
    TORCH_CHECK(false, "Given output_size=(", output_height, ", ", output_width,
                "), kernel_size=(", kernel_height, ", ", kernel_width,
                "), dilation=(", dilation_height, ", ", dilation_width,
                "), padding=(", pad_height, ", ", pad_width, "), stride=(",
                stride_height, ", ", stride_width,
                "), expected size of input's dimension 2 to match the "
                "calculated number of ",
                "sliding blocks ", n_blocks_height, " * ", n_blocks_width,
                " = ", (n_blocks_height * n_blocks_width),
                ", but got input.size(2)=", input_length, ".");
  }

  TORCH_CHECK(n_blocks_height >= 1 && n_blocks_width >= 1,
...
inline void im2col_shape_check(const at::Tensor& input,
                               const at::Tensor& grad_output,
                               int64_t kernel_height, int64_t kernel_width,
                               int64_t dilation_height, int64_t dilation_width,
                               int64_t pad_height, int64_t pad_width,
  ...
  TORCH_CHECK((ndim == 3 && input.size(0) && valid_dims) ||
  ...
  int64_t output_height =
      div_rtn<int64_t>(input_height + 2 * pad_height -
                           (dilation_height * (kernel_height - 1) + 1),
                       stride_height) +
      1;
  int64_t output_width =
      div_rtn<int64_t>(input_width + 2 * pad_width -
                           (dilation_width * (kernel_width - 1) + 1),
                       stride_width) +
      1;

  if (output_height < 1 || output_width < 1) {
    TORCH_CHECK(false, "Given input with spatial size (", input_height, ", ",
                input_height, "), kernel_size=(", kernel_height, ", ",
                kernel_width, "), dilation=(", dilation_height, ", ",
                dilation_width, "), padding=(", pad_height, ", ", pad_width,
                "), calculated shape of the array of sliding blocks as (",
                output_height, ", ", output_width,
...
void col2im_out_gcu_template(at::Tensor& output, const at::Tensor& input_,
                             at::IntArrayRef output_size,
                             at::IntArrayRef kernel_size,
                             at::IntArrayRef dilation, at::IntArrayRef padding,
                             at::IntArrayRef stride) {
  at::TensorArg input_arg{input_, "input", 1};
  ...
  col2im_shape_check(input_, at::Tensor(), output_height, output_width,
                     kernel_height, kernel_width, dilation_height,
                     dilation_width, pad_height, pad_width, stride_height,
                     stride_width);
...
static void im2col_out_gcu_template(
    at::Tensor& output, const at::Tensor& input_, at::IntArrayRef kernel_size,
  ...
  im2col_shape_check(input_, at::Tensor(), kernel_height, kernel_width,
...
at::Tensor& col2im_out_shape_infer(const at::Tensor& input,
                                   at::IntArrayRef output_size,
                                   at::IntArrayRef kernel_size,
                                   at::IntArrayRef dilation,
                                   at::IntArrayRef padding,
                                   at::IntArrayRef stride, at::Tensor& output) {
  col2im_out_gcu_template(output, input, output_size, kernel_size, dilation,
                          padding, stride);
  return output;
}

at::Tensor col2im_shape_infer(const at::Tensor& input,
                              at::IntArrayRef output_size,
                              at::IntArrayRef kernel_size,
                              at::IntArrayRef dilation, at::IntArrayRef padding,
                              at::IntArrayRef stride) {
...
at::Tensor& im2col_out_shape_infer(const at::Tensor& input,
                                   at::IntArrayRef kernel_size,
                                   at::IntArrayRef dilation,
                                   at::IntArrayRef padding,
                                   at::IntArrayRef stride, at::Tensor& output) {
  im2col_out_gcu_template(output, input, kernel_size, dilation, padding,
                          stride);
  return output;
}

at::Tensor im2col_shape_infer(const at::Tensor& input,
                              at::IntArrayRef kernel_size,
                              at::IntArrayRef dilation, at::IntArrayRef padding,
                              at::IntArrayRef stride) {
  at::Tensor output = at::empty_like(input, LEGACY_CONTIGUOUS_MEMORY_FORMAT);
