...
class autocast(torch.amp.autocast_mode.autocast):
    ...
    ``torch.gcu.amp.autocast(args...)`` is equivalent to ``torch.autocast("gcu", args...)``
    """
    ...
    def __init__(self, enabled: bool = True, dtype: torch.dtype = torch.float16, cache_enabled: bool = True):
        if torch._jit_internal.is_scripting():
            ...
            return
        super().__init__("gcu", enabled=enabled, dtype=dtype, cache_enabled=cache_enabled)
...
def _cast(value, dtype):
    if isinstance(value, torch.Tensor):
        is_eligible = (value.is_floating_point() and value.device.type == 'gcu' and (value.dtype is not torch.float64))
...
def custom_fwd(fwd=None, **kwargs):
    ...
    Args:
        cast_inputs (:class:`torch.dtype` or None, optional, default=None):  If not ``None``,
            ...
            floating-point GCU Tensors to the target dtype (non-floating-point Tensors are not affected),
    ...
    def decorate_fwd(*args, **kwargs):
        if cast_inputs is None:
            args[0]._fwd_used_autocast = torch_gcu._C.is_autocast_enabled()
            return fwd(*args, **kwargs)
        else:
            autocast_context = torch_gcu._C.is_autocast_enabled()
