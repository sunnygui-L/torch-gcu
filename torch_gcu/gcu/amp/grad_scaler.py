...
class _GCUMultiDeviceReplicator(_MultiDeviceReplicator):
    """
    Lazily serves copies of a tensor to requested devices.  Copies are cached per-device.
    """

    def __init__(self, master_tensor: torch.Tensor) -> None:
        if not master_tensor.is_gcu:
            raise ValueError("Device type of master_tensor should be gcu.")
...
class GradScaler(Cuda_GradScaler):
    ...
    * If no infs/NaNs are found, ``scaler.step(optimizer)`` runs the underlying ``optimizer.step()`` as usual.
    ...
    def __init__(self,
                 ...
                 enabled=True):
        if enabled and (not torch.gcu.is_available()):
            warnings.warn("torch_gcu.amp.GradScaler is enabled, but GCU is not available.  Disabling.")
            self._enabled = False
        else:
    ...
    def scale(self, outputs):
        ...
        if isinstance(outputs, torch.Tensor):
            if not outputs.is_gcu:
                raise ValueError("Device type of outputs should be gcu.")
        ...
        stash: List[_GCUMultiDeviceReplicator] = []  # holds a reference that can be overwritten by apply_scale

        def apply_scale(val):
            if isinstance(val, torch.Tensor):
                if not val.is_gcu:
                    raise ValueError("Device type of val should be gcu.")
                if len(stash) == 0:
                    if self._scale is None:
                        self._lazy_init_scale_growth_tracker(val.device)
                    if self._scale is None:
                        raise RuntimeError("_scale is None.")
                    stash.append(_GCUMultiDeviceReplicator(self._scale))
    ...
    def _unscale_grads_(self, optimizer, inv_scale, found_inf, allow_fp16):
        per_device_found_inf = _GCUMultiDeviceReplicator(found_inf)
        per_device_inv_scale = _GCUMultiDeviceReplicator(inv_scale)
    ...
    def unscale_(self, optimizer):
        ...
        .. note::
            :meth:`unscale_` does not incur a CPU-GCU sync.
    ...
    def update(self, new_scale=None):
        ...
        Args:
            new_scale (float or :class:`torch.gcu.FloatTensor`, optional, default=None):  New scale factor.
        ...
        if new_scale is not None:
            ...
            if isinstance(new_scale, float):
                ...
                reason = "new_scale should be a float or a 1-element torch.gcu.FloatTensor with requires_grad=False."
                if not isinstance(new_scale, torch.gcu.FloatTensor):  # type: ignore[attr-defined]
