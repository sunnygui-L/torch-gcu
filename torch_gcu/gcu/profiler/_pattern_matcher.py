...
class ExtraGCUCopyPattern(Pattern):
    ...
    def __init__(self, prof: profile, should_benchmark: bool = False):
        super().__init__(prof, should_benchmark)
        self.name = "Extra GCU Copy Pattern"
        self.description = "Filled a CPU tensor and immediately moved it to GCU. Please initialize it on GCU."
    ...
    def match(self, event):
        ...
        # Up one level
        event = event.parent
        # In case of transfer_to_gcu
        if event.parent and torch_gcu.is_transfer_to_gcu and "decorated" in event.parent.name and "transfer_to_gcu" in event.name:
            event = event.parent
        # Using tensor.gcu() rather than tensor.to('gcu') involves an additional wrap_tensor_to layer atop the built-in method in the call stack.
        if event.parent and "wrap_tensor_to" in event.parent.name:
            event = event.parent
        if event is None:
            return False
    ...
    def benchmark(self, events: List[_ProfilerEvent]):
        ...
        for shape in shapes_factor_map:
            ...
            to_timer = benchmark.Timer(
                stmt='torch.ones(size).to("gcu")', globals={"size": size}
            )
            de_timer = benchmark.Timer(
                stmt='torch.ones(size, device="gcu")', globals={"size": size}
            )
...
class FP32MatMulPattern(Pattern):
    def __init__(self, prof: profile, should_benchmark: bool = False):
        ...
        self.description = (
            ...
            "Please enable TF32 by setting 'torch.backends.gcu.matmul.allow_tf32 = True'"
        )
        self.url = "https://pytorch.org/docs/stable/notes/gcu.html#tensorfloat-32-tf32-on-ampere-devices"
    ...
    def skip(self):
        if torch.version.hip is not None:
            ...
            has_tf32 = all(int(arch[3:]) >= 80 for arch in torch.gcu.get_arch_list())
    ...
    def benchmark(self, events: List[_ProfilerEvent]):
        shapes_factor_map = {input_shapes(event): 0.0 for event in events}
        for shape in shapes_factor_map:
            matrixA = torch.randn(shape[0], device="gcu", dtype=torch.float32)
            matrixB = torch.randn(shape[1], device="gcu", dtype=torch.float32)
            ...
            tf32_timer = benchmark.Timer(
                stmt="torch.mm(matrixA, matrixB)",
                setup="torch.backends.gcu.matmul.allow_tf32 = True",
                globals={"matrixA": matrixA, "matrixB": matrixB},
            )
            torch.backends.gcu.matmul.allow_tf32 = False
...
class MatMulDimInFP16Pattern(Pattern):
    ...
    def match(self, event: _ProfilerEvent):
        def multiple_of(shapes, multiple):
        ...
        if arg_dtype in (torch.bfloat16, torch.half) and not multiple_of(
    ...
    def benchmark(self, events: List[_ProfilerEvent]):
        ...
        shapes_factor_map = {input_shapes(event): 0.0 for event in events}
        for shape in shapes_factor_map:
            matrixA = torch.randn(shape[0], device="gcu", dtype=torch.float16)
            matrixB = torch.randn(shape[1], device="gcu", dtype=torch.float16)
            not_aligned_dim_timer = benchmark.Timer(
                stmt="torch.mm(matrixA, matrixB)",
                globals={"matrixA": matrixA, "matrixB": matrixB},
            )
            matrixA = torch.randn(
                closest_multiple(shape[0], 8), device="gcu", dtype=torch.float16
            )
            matrixB = torch.randn(
                closest_multiple(shape[1], 8), device="gcu", dtype=torch.float16
            )
...
def report_all_anti_patterns(
    ...
    report_dict: Dict = {}
    anti_patterns = [
        ExtraGCUCopyPattern(prof, should_benchmark),
    ...
    if json_report_dir is not None:
        ...
        if os.path.exists(json_report_path):
            with open(json_report_path) as f:
                ...
                existing_report.update(report_dict)
                report_dict = existing_report
