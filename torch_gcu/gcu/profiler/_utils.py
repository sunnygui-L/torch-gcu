...
from torch_gcu.gcu.autograd.profiler import profile
from torch_gcu.gcu.profiler import DeviceType
...
class BasicEvaluation:
    def __init__(self, prof: profile):
        ...
        self.gcu_events: List[_KinetoEvent] = []
    ...
    def compute_queue_depth(self):
        ...
        This will return a list of Interval of queue depth data of gcu launch and kernels.
        """
        assert self.profile.kineto_results is not None
        gcu_event_list = self.profile.kineto_results.events()

        def is_gcu_launch_kernel(e):
            # TODO: find a better way to identify gcuLaunchKernel
            return e.name == "topsLaunchKernel"

        def is_gcu_kernel(e):
            # TODO: find a better way to identify GCU Kernel
            return e.device_type() == DeviceType.PrivateUse1 and "mem" not in e.name.lower()

        gcu_launch_events = sorted(
            (e for e in gcu_event_list if is_gcu_launch_kernel(e)),
            key=lambda x: x.start_ns(),
        )
        gcu_kernel_events = sorted(
            (e for e in gcu_event_list if is_gcu_kernel(e)),
            key=lambda x: x.start_ns(),
        )

        self.gcu_events = sorted(
            gcu_launch_events + gcu_kernel_events, key=lambda x: x.start_ns()
        )

        kernel_mapping: Dict[_KinetoEvent, int] = {}
        last_mapped_kernel = 0
        for gcu_launch_event in gcu_launch_events:
            index = index_of_first_match(
                gcu_kernel_events,
                lambda x: x.linked_correlation_id()
                == gcu_launch_event.linked_correlation_id(),
                start=last_mapped_kernel,
            )
            kernel_mapping[gcu_launch_event] = index
            last_mapped_kernel = index if index is not None else last_mapped_kernel

        current_kernel_index = 0
        spawned_kernel_index = -1

        all_events = gcu_launch_events + gcu_kernel_events + self.events
        ...
        for event in all_events:
            ...
            while (
                current_kernel_index < len(gcu_kernel_events)
                and (gcu_kernel_events[current_kernel_index].start_ns())
    ...
    def rank_events(self, length):
        ...
        while i < len(qd_values):
            if qd_values[i] > bottom_threshold:
                i += 1
                continue
            for j in range(i + 1, len(qd_values)):
                ...
                next_minimum_idx = index_of_first_match(
                    qd_values, lambda x: x <= bottom_threshold, start=j
                )
                peak_idx = argmax(qd_values, start=j, end=next_minimum_idx)

                # if is a valid peak, we add to list and continue
                if peak_idx is not None and qd_values[peak_idx] >= top_threshold:
...
# Provide an OSS workaround for gcugraphs + CUPTI issue
# https://github.com/pytorch/pytorch/issues/75504
def _init_for_gcu_graphs():
    from torch_gcu.gcu.autograd.profiler import profile
