...
import torch
import torch_gcu
import torch_gcu.gcu.autograd.profiler as prof
from torch._C import _get_privateuse1_backend_name
from torch_gcu._C._profiler import (
...
from torch_gcu.gcu.autograd import kineto_available
from torch.autograd import ProfilerActivity
from torch_gcu.gcu.profiler._memory_profiler import MemoryProfile, MemoryProfileTimeline
...
def supported_activities():
    ...
    Note: profiler uses TOPSPTI library to trace on-device GCU kernels.
    In case when GCU is enabled but TOPSPTI is not available, passing
    ``ProfilerActivity.PrivateUse1`` to profiler results in using the legacy GCU
    profiling code (same as in the legacy ``torch.autograd.profiler``).
    This, in turn, results in including GCU time in the profiler table output,
    but not in the JSON trace.
    """
    return torch_gcu.gcu.autograd._supported_activities()
...
class _KinetoProfile:
    ...
    Args:
        activities (iterable): list of activity groups (CPU, GCU) to use in profiling, supported values:
            ``torch.profiler.ProfilerActivity.CPU``, ``torch.profiler.ProfilerActivity.PrivateUse1``,
            ``torch.profiler.ProfilerActivity.XPU``.
            Default value: ProfilerActivity.CPU and (when available) ProfilerActivity.PrivateUse1
    ...
    def start_trace(self):
        ...
        if kineto_available():
            ...
            #     if inductor_config.triton.gcugraphs:
            #         os.environ["DISABLE_TOPSPTI_LAZY_REINIT"] = "1"
            #         self.add_metadata_json("DISABLE_TOPSPTI_LAZY_REINIT", "1")
            #         os.environ["TEARDOWN_TOPSPTI"] = "0"
    ...
    def export_stacks(self, path: str, metric: str = "self_cpu_time_total"):
        ...
        Args:
            ...
            metric (str): metric to use: "self_cpu_time_total" or "self_gcu_time_total"
        """
        assert self.profiler
    ...
    def toggle_collection_dynamic(
        ...
        (CPU) and GCU activity supported in Kineto
        ...
        .. code-block:: python
            ...
            with torch.profiler.profile(
                ...
                code_to_profile_0()
                // turn off collection of all GCU activity
                p.toggle_collection_dynamic(False, [torch.profiler.ProfilerActivity.PrivateUse1])
                code_to_profile_1()
                // turn on collection of all GCU activity
                p.toggle_collection_dynamic(True, [torch.profiler.ProfilerActivity.PrivateUse1])
                code_to_profile_2()
            print(p.key_averages().table(
                sort_by="self_gcu_time_total", row_limit=-1))
        """
    ...
    def add_metadata(self, key: str, value: str):
        ...
        torch_gcu.gcu.autograd._add_metadata_json(key, wrapped_value)
    ...
    def add_metadata_json(self, key: str, value: str):
        ...
        into the trace file
        """
        torch_gcu.gcu.autograd._add_metadata_json(key, value)
    ...
    def _get_distributed_info(self):
        ...
        # TODO: support eccl version
        # if backend == "eccl":
        #     nccl_version = torch.gcu.eccl.version()
        #     dist_info["eccl_version"] = ".".join(str(v) for v in eccl_version)
        return dist_info
    ...
    def export_memory_timeline(self, path: str, device: Optional[str] = None) -> None:
        ...
        if device is None and self.use_device and self.use_device != "gcu":
            device = self.use_device + ":0"

        if device is None:
            device = "gcu:0" if torch.gcu.is_available() else "cpu"
    ...
    def __init__(
        ...
        *,
        ...
        if use_cuda is not None:
            ...
            if use_cuda:
                activities_set.add(ProfilerActivity.PrivateUse1)
            elif ProfilerActivity.PrivateUse1 in activities_set:
                activities_set.remove(ProfilerActivity.PrivateUse1)
